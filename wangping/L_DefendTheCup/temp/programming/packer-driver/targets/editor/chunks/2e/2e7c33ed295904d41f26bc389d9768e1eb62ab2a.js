    ...
    execute: function () {
      ...
      _export("FlowField", FlowField = class FlowField extends (_crd && Singleton === void 0 ? (_reportPossibleCrUseOfSingleton({
        ...
        updateFlowField(playerWorldPos) {
          ...
          const playerGridPos = this.worldToGrid(playerWorldPos); // 2. 重置所有格子代价
          ...
          const queue = [playerGridPos];
          this._grid[playerGridPos.x][playerGridPos.y].cost = 0;

          while (queue.length > 0) {
            const current = queue.shift();
            const currentCost = this._grid[current.x][current.y].cost; // 检查4个相邻格子（上、下、左、右）

            const neighbors = [new Vec2(current.x, current.y + 1), new Vec2(current.x, current.y - 1), new Vec2(current.x + 1, current.y), new Vec2(current.x - 1, current.y)];
            ...
            for (const neighbor of neighbors) {
              if (this.isValidGrid(neighbor)) {
                ...
                if (newCost < this._grid[neighbor.x][neighbor.y].cost) {
                  ...
                  const dir = new Vec2(current.x - neighbor.x, current.y - neighbor.y).normalize();
