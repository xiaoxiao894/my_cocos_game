      ...
      _export("PlayerManager", PlayerManager = (_dec = ccclass('PlayerManager'), _dec2 = requireComponent(_crd && Actor === void 0 ? (_reportPossibleCrUseOfActor({
        ...
        monsterHitEffect(monsters) {
          for (let i = 0; i < monsters.length; i++) {
            ...
            if (!effect) continue; // 设置特效位置为怪物当前位置
        ...
        getIcon(dt) {
          const newMeats = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          ...
          const maxDistanceXZ = 33; // 合并新掉落 + 上次未收集的

          const allMeats = [...newMeats, ...this._uncollectedMeat];
          ...
          for (let i = 0; i < allMeats.length; i++) {
            ...
            const distSqrXZ = dx * dx + dz * dz; // 超出范围：暂存等待下一次
