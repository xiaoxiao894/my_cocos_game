    ...
    execute: function () {
      ...
      _export("PartnerManager", PartnerManager = (_dec = ccclass('PartnerManager'), _dec2 = property(Prefab), _dec3 = property(Prefab), _dec4 = property(Prefab), _dec(_class = (_class2 = class PartnerManager extends Component {
        ...
        init() {
          ...
          const poolCount = 5;
          const bigSkillsPrefabPoolCount = 100; // 普通攻击
        ...
        update(dt) {
          ...
          const partnerWorldPos = this.node.worldPosition; // 仅在目标没有锁定时，更新目标 
        ...
        fireAtTarget() {
          ...
          const direction = monsterPos.clone().subtract(selfPos).normalize(); //  中点位置

          const midPoint = selfPos.clone().add(monsterPos).multiplyScalar(0.5); //  偏移位置（从中点向角色方向偏移）
        ...
        playPartnerAnim(name) {
          ...
          const state = anim == null ? void 0 : anim.getState(name);
        ...
        explosiveSpecialEffects(target, name) {
          ...
          const worldPos = target instanceof Node ? target.worldPosition : target; //  生成爆炸特效
          ...
          const anim = skillExplosion == null || (_skillExplosion$child = skillExplosion.children[0]) == null ? void 0 : _skillExplosion$child.getComponent(Animation);
        ...
        isTargetOnRight(self, target) {
          ...
          const targetPos = target.worldPosition; // 角色的 forward 方向（Z轴向前）

          const forward = self.forward.clone().normalize(); // 朝目标方向的向量

          const toTarget = targetPos.clone().subtract(selfPos).normalize(); // 角色右方向 = forward × up
        ...
        releaseMajorSkills() {
          ...
          const itemAreaManager = this.node.parent.getComponent(_crd && ItemAreaManager === void 0 ? (_reportPossibleCrUseOfItemAreaManager({
            error: Error()
          }), ItemAreaManager) : ItemAreaManager);
          const center = this.node.worldPosition.clone(); // 技能中心点为当前角色位置

          const directionAngleDeg = (_itemAreaManager$dire = itemAreaManager.directionAngleDeg) != null ? _itemAreaManager$dire : 0;
          const directionAngleRad = directionAngleDeg * Math.PI / 180; // 朝向向量（forward），单位向量，决定技能释放的主方向（默认Z正方向）

          const forward = new Vec3(Math.sin(directionAngleRad), 0, Math.cos(directionAngleRad)).normalize(); // 外层循环：每一圈技能生成逻辑
          ...
          for (let ring = 0; ring < itemAreaManager.ringCount; ring++) {
            ...
            for (let i = 0; i < selectCount && i < candidateCount; i++) {
              const idx = indices[i]; // 当前角度在扇形中的位置

              const angleStep = itemAreaManager.spreadAngle / (candidateCount - 1);
              const angleDeg = -itemAreaManager.spreadAngle / 2 + angleStep * idx;
              const angleRad = angleDeg * Math.PI / 180; // 将角度旋转后的方向向量

              const dir = new Vec3(Math.sin(directionAngleRad + angleRad), 0, Math.cos(directionAngleRad + angleRad)).normalize(); // 生成点的基础位置

              const basePos = center.clone().add(dir.multiplyScalar(baseRadius)); // 加上一个位置扰动偏移（模拟不规则性）

              const randomOffset = new Vec3((Math.random() - 0.5) * 2 * itemAreaManager.radiusRandomOffset, 0, (Math.random() - 0.5) * 2 * itemAreaManager.radiusRandomOffset);
