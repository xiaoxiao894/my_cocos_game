      ...
      _export("default", MonsterBorn = class MonsterBorn {
        static getWorldBornPos() {
          ...
          for (let i = 0; i < maxTry; i++) {
            const angle = Math.random() * 2 * Math.PI; //  在中心点为圆心的固定半径圆环上生成点

            const offset = new Vec3(Math.cos(angle) * radius, 0, Math.sin(angle) * radius);
            const targetWorld = centerWorld.clone().add(offset); //  将该点转为屏幕坐标

            const screenPos = cameraMain.worldToScreen(targetWorld);
            const screenSize = view.getVisibleSize(); //  仅接受屏幕外的点
        ...
        static randomAngle() {
          const pos = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          ...
          const centerAngle = Math.atan2(z, x) + Math.PI; // 确定边界条件

          const atXBoundary = Math.abs(x) > this.xBoundary;
          const atZBoundary = Math.abs(z) > this.zBoundary; // 计算随机角度范围
