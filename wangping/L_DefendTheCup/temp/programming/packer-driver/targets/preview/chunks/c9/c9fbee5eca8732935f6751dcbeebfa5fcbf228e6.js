    ...
    execute: function () {
      ...
      _export("default", MonsterManager = (_dec = ccclass('MonsterManager'), _dec2 = property(Node), _dec3 = property(Node), _dec(_class = (_class2 = class MonsterManager extends Component {
        ...
        init() {
          ...
          for (var _i = 0; _i < this.initMonsterPos.length; _i++) {
            var {
              pos
            } = this.initMonsterPos[_i];
            (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              error: Error()
        ...
        creatMonster(isDissolveOnce, isAfferentPos, pos) {
          ...
          var pool = this._monsterPools[rad];
          ...
          if (isAfferentPos) {
            ...
            var birthPoints = this.monsterBirthPointCon.children;
          ...
          var bloodNode = this._bloodPool.getItem();
        ...
        getWorldBornPos() {
          //有效范围
          var fixedY = 1; //欠地图边界判断
          //随机角度

          var radAngle = Math.random() * 2 * Math.PI; //console.log(`monster radAngle ${radAngle}`);
          ...
          var uiPos = this.getRayRectangleIntersection(radAngle);
          ...
          if (uiPos) {
            ...
            var queryTrigger = true; // if (PhysicsSystem.instance.raycastClosest(ray, mask, maxDistance, queryTrigger)) {

            var raycastClosestResult = PhysicsSystem.instance.raycastClosestResult;
            var hitPoint = raycastClosestResult.hitPoint; //console.log(`monster worldPos ${hitPoint}`);   
            //pos.y = fixedY;
            ...
            if (this._monsterPools.length > 0) {
              ...
              if (currentMonsterCount < maxMonsterCount) {
                ...
                if (this._bornTime > this._bornTimeLimit) {
                  ...
                  var spawnCount = Math.min(this._bornSpeed, maxMonsterCount - currentMonsterCount);
        ...
        getDrops() {
          var newList = this._dropList.splice(0, this._dropList.length);
        ...
        getSurroundedMonsters() {
          ...
          for (var i = 0; i < this.monsterParent.children.length; i++) {
            var monster = this.monsterParent.children[i];
            if (!monster) continue;
