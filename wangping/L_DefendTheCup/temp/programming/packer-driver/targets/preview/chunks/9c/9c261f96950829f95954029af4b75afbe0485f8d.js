  ...
  return {
    ...
    execute: function () {
      ...
      _export("BossTipConMananger", BossTipConMananger = (_dec = ccclass('BossTipConMananger'), _dec2 = property(Camera), _dec3 = property(Node), _dec(_class = (_class2 = class BossTipConMananger extends Component {
        ...
        update() {
          for (var arrow of this.arrowTargets) {
        ...
        addTarget(target3D) {
          ...
          var bossTipNode = instantiate(prefab);
          bossTipNode.setParent(this.node);
          bossTipNode.setSiblingIndex(999);
          var tipTransform = bossTipNode.getComponent(UITransform);
        ...
        removeTarget(target3D) {
          var index = this.arrowTargets.findIndex(a => a.target3D === target3D);
        ...
        updateArrowForTarget(_ref) {
          ...
          var halfW = canvasTransform.contentSize.width / 2;
          var halfH = canvasTransform.contentSize.height / 2; //  判断是否在视野内

          var isInView = _tempUINodePos.x >= -halfW + this.margin && _tempUINodePos.x <= halfW - this.margin && _tempUINodePos.y >= -halfH + this.margin && _tempUINodePos.y <= halfH - this.margin;
          var arrow = bossTipNode.getChildByName("Arrow");
          ...
          if (isInView) {
            ...
            var dist = 100;
            var angleRad = bossTipNode.angle * Math.PI / 180;
            ...
            // 屏幕外：箭头指向目标方向
            var dir = _tempUINodePos.clone().normalize();

            if (arrow) arrow.angle = Math.atan2(dir.y, dir.x) * 180 / Math.PI + 90;
            var clampedX = Math.min(Math.max(_tempUINodePos.x, -halfW + this.arrowMargin), halfW - this.arrowMargin);
            var clampedY = Math.min(Math.max(_tempUINodePos.y, -halfH + this.arrowMargin), halfH - this.arrowMargin);
