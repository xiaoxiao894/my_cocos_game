  ...
  return {
    ...
    execute: function () {
      ...
      _export("MonsterItem", MonsterItem = (_dec = ccclass('MonsterItem'), _dec2 = property(SkeletalAnimation), _dec3 = property(CCString), _dec4 = property(CCString), _dec5 = property(CCInteger), _dec6 = property(_crd && DissolveEffect === void 0 ? (_reportPossibleCrUseOfDissolveEffect({
        ...
        init(index, bloodNode, isDissolveOnce) {
          if (bloodNode === void 0) {
            bloodNode = null;
          }
          ...
          if (this._bloodNode) {
            var bar = this._bloodNode.getComponent(ProgressBar);
        ...
        deathAni(isPlayer) {
          ...
          if (isPlayer) {
            ...
            var playerPos = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              error: Error()
            ...
            var targetNode = new Node("Temp");
            targetNode.setWorldPosition(knockbackFinalPos);
            var isInsideDoor = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              error: Error()
        ...
        updateIconPos() {
          if (this.node.name == "Mantis") {
            var randomIconNum = (_crd && MathUtil === void 0 ? (_reportPossibleCrUseOfMathUtil({
              error: Error()
            }), MathUtil) : MathUtil).getRandom(3, 5);
            var worldPos = this.node.getWorldPosition().clone();

            for (var i = 0; i < randomIconNum; i++) {
              var randius = 3;
        ...
        setPreferredVelocities(dt) {
          ...
          var moveTarget = v2(playerPos.x, playerPos.z);
          var worldTarget = new Vec3(playerPos.x, 0, playerPos.z);
          var isPlayerInDoorArea = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          ...
          if (isPlayerInDoorArea) {
            ...
            if (nearestGuardrail && !this._assignedGuardrail) {
              ...
              moveTarget = v2(_guardrailPos.x, _guardrailPos.z);
              worldTarget = new Vec3(_guardrailPos.x, 0, _guardrailPos.z);
            }
          } else {
            if (this._assignedGuardrail) {
              ...
              var count = this._assignedGuardrail.attackingMonsterCount || 0;
              var _guardrailPos2 = this._assignedGuardrail.node.worldPosition;
              moveTarget = v2(_guardrailPos2.x, _guardrailPos2.z);
              worldTarget = new Vec3(_guardrailPos2.x, 0, _guardrailPos2.z);

              if (count === 0) {
                var _monsterPos = this.node.worldPosition.clone();

                var _dx = playerPos.x - _monsterPos.x;

                var _dz = playerPos.z - _monsterPos.z;

                var _distSqr = _dx * _dx + _dz * _dz;

                var playerRange = 0.3;

                if (_distSqr <= playerRange * playerRange) {
                  this._assignedGuardrail = null;
                  this._hasCountedAttack = false;
                  this._noMove = false;
          ...
          if (agent && agentPos) {
            ...
            var hasArrived = distanceToTargetSquared <= 16;
            ...
            if (forward.lengthSqr() > 0.0001) {
              var currentRotation = this.node.worldRotation.clone();
        ...
        attack() {
          ...
          if (this._assignedGuardrail) {
            ...
            if (this._assignedGuardrail.node.name.includes("Door")) {
              ...
              //围栏
              var fenceNode = this._assignedGuardrail.node.getChildByName("B");
        ...
        attackOver() {
          if (this._isAttackPlayer) {
            ...
            var monsterPos = this.node.getWorldPosition();
            var distSqr = Vec3.squaredDistance(playerPos, monsterPos);
