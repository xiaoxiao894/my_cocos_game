    ...
    execute: function () {
      ...
      _export("WeaponManager", WeaponManager = (_dec = ccclass('WeaponManager'), _dec2 = property(CCFloat), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(Node), _dec6 = property(Node), _dec7 = property(Node), _dec(_class = (_class2 = class WeaponManager extends Component {
        ...
        syncSlotOccupiedState() {
          for (var i = 0; i < this.weaponPos.length; i++) {
            var pos = this.weaponPos[i];
            var occupied = false;

            for (var child of this.weapons.children) {
              var localPos = this.weapons.inverseTransformPoint(new Vec3(), child.getWorldPosition());
        ...
        createWeapon() {
          ...
          for (var i = 0; i < this.weaponPos.length; i++) {
            var expectedPos = this.weaponPos[i];
            var isOccupied = false;

            for (var child of this.weapons.children) {
              var worldPos = child.getWorldPosition();
              var localPos = this.weapons.inverseTransformPoint(new Vec3(), worldPos);
          ...
          var end = this.weaponPos[slotIndex].clone();
          var duration = 0.6;
        ...
        spawnAndAnimateWeapon(prefab, end, duration, slotIndex) {
          ...
          tween(weapon).to(duration, {
            ...
            var worldPos = weapon.getWorldPosition();
            var finalLocal = this.weapons.inverseTransformPoint(new Vec3(), worldPos);
        ...
        playerWeaponRotationAni(weapon) {
          var duration = 0.6;
          var startEuler = weapon.children[0].eulerAngles.clone();
        ...
        automaticallyTransmitAni() {
          ...
          var children = this.weapons.children;
          if (children.length === 0) return;
          var lastWeapon = children[children.length - 1];
          if (!lastWeapon || !lastWeapon.isValid) return;
          this._isTransmitting = true;
          var slotIndex = lastWeapon.__slotIndex;

          if (slotIndex !== undefined) {
            this._slotOccupied[slotIndex] = false;
          }

          var worldStartPos = lastWeapon.getWorldPosition();
          var worldRotation = lastWeapon.children[0].getWorldRotation();
          ...
          var jumpTarget = this.transferStart.getWorldPosition();
          var jumpMid = new Vec3((worldStartPos.x + jumpTarget.x) / 2, Math.max(worldStartPos.y, jumpTarget.y) + 2, (worldStartPos.z + jumpTarget.z) / 2);
          var controller = {
            t: 0
          };
          tween(controller).to(0.4, {
            ...
            onUpdate: () => {
              var t = controller.t;
              var oneMinusT = 1 - t;
              var pos = new Vec3(oneMinusT * oneMinusT * worldStartPos.x + 2 * oneMinusT * t * jumpMid.x + t * t * jumpTarget.x, oneMinusT * oneMinusT * worldStartPos.y + 2 * oneMinusT * t * jumpMid.y + t * t * jumpTarget.y, oneMinusT * oneMinusT * worldStartPos.z + 2 * oneMinusT * t * jumpMid.z + t * t * jumpTarget.z);
              lastWeapon.setWorldPosition(pos);
            }
          }).call(() => {
            var basePos = lastWeapon.getWorldPosition();
            tween(lastWeapon).to(0.15, {
              ...
              var center = this.transferCenter.getWorldPosition();
              var end = this.transferEnd.getWorldPosition();
              tween(lastWeapon).to(0.6, {
                ...
                var total = this.minionWeaponCon.children.length;
                var itemsPerRow = 4;
                var spacingZ = 1.3;
                var spacingY = 1;
                var col = total % itemsPerRow;
                var row = Math.floor(total / itemsPerRow);
                var localTarget = new Vec3(0, row * spacingY, col * spacingZ);
                var worldTarget = this.minionWeaponCon.getWorldPosition().clone();
                var worldRot = this.minionWeaponCon.getWorldRotation();
                var worldScale = this.minionWeaponCon.getWorldScale();
                var mat = new Mat4();
                Mat4.fromRTS(mat, worldRot, worldTarget, worldScale);
                var targetPos = Vec3.transformMat4(new Vec3(), localTarget, mat);
                var finalStart = lastWeapon.getWorldPosition();
                var finalControl = new Vec3((finalStart.x + targetPos.x) / 2, Math.min(Math.max(finalStart.y, targetPos.y) + 2, 5), (finalStart.z + targetPos.z) / 2);
                var finalController = {
                  t: 0
                };
                tween(finalController).to(0.4, {
                  ...
                  easing: 'quadInOut',
                  onUpdate: () => {
                    var t = finalController.t;
                    var oneMinusT = 1 - t;
                    var pos = new Vec3(oneMinusT * oneMinusT * finalStart.x + 2 * oneMinusT * t * finalControl.x + t * t * targetPos.x, oneMinusT * oneMinusT * finalStart.y + 2 * oneMinusT * t * finalControl.y + t * t * targetPos.y, oneMinusT * oneMinusT * finalStart.z + 2 * oneMinusT * t * finalControl.z + t * t * targetPos.z);
                    lastWeapon.setWorldPosition(pos);
                  }
                }).call(() => {
                  // 被传送过来的武器
        ...
        getLastPositionWeapon() {
          var weapons = this.weapons.children.filter(w => w && w.isValid);
