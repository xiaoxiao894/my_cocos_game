    ...
    execute: function () {
      ...
      _export("MinionConManager", MinionConManager = (_dec = ccclass('MinionConManager'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec(_class = (_class2 = class MinionConManager extends Component {
        ...
        checkQueueAndRefill() {
          ...
          if (!this.node) return;
          var children = this.node.children; // 已有小兵自动前进补位

          var _loop = function _loop() {
            var _tweenTarget;

            var minion = children[i];
            var targetPos = _this.queuePosition[i];
            if (!minion) return 0; // continue

            var minionManager = minion == null ? void 0 : minion.children[0].getComponent(_crd && MinionManager === void 0 ? (_reportPossibleCrUseOfMinionManager({
              error: Error()
            }), MinionManager) : MinionManager);
            var currentPos = minion.position; // 如果已经在位置上，确保状态为 Idle
            ...
            if ((_tweenTarget = minion._tweenTarget) != null && _tweenTarget.equals(targetPos)) return 0; // continue
            ...
            var duration = Vec3.distance(currentPos, targetPos) / _this.moveSpeed;
          ...
          if (children.length < this.queuePosition.length && this._spawnTimer >= this.spawnCooldown) {
            ...
            var targetPos = this.queuePosition[children.length - 1];
            var duration = Vec3.distance(this.birthPoint, targetPos) / this.moveSpeed;
        ...
        goOutMinion() {
          ...
          var validWeapons = this.minionWeaponCon.children.filter(w => w["__delivered"] && !w["__used"]);
          if (validWeapons.length <= 0) return;
          this._isDeliveringMinion = true;
          var weapon = validWeapons[validWeapons.length - 1];
          ...
          var minion = this.node.children[0];
          ...
          weapon["__used"] = true;
          var weaponStart = weapon.getWorldPosition();
          var minionWorld = minion.getWorldPosition();
          var weaponTarget = minionWorld.clone().add(new Vec3(0, 2, 0));
          var controlPoint = new Vec3((weaponStart.x + weaponTarget.x) / 2, Math.max(weaponStart.y, weaponTarget.y) + 5, (weaponStart.z + weaponTarget.z) / 2); // 先放到世界空间中进行动画
          ...
          tween(controller).to(0.4, {
            ...
            onUpdate: () => {
              ...
              var t = controller.t;
              var oneMinusT = 1 - t;
              var pos = new Vec3(oneMinusT * oneMinusT * weaponStart.x + 2 * oneMinusT * t * controlPoint.x + t * t * weaponTarget.x, oneMinusT * oneMinusT * weaponStart.y + 2 * oneMinusT * t * controlPoint.y + t * t * weaponTarget.y, oneMinusT * oneMinusT * weaponStart.z + 2 * oneMinusT * t * controlPoint.z + t * t * weaponTarget.z);
            ...
            var worldPos = weapon.worldPosition;
            var effectPrefab = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              error: Error()
            }), DataManager) : DataManager).Instance.prefabMap.get((_crd && EntityTypeEnum === void 0 ? (_reportPossibleCrUseOfEntityTypeEnum({
              error: Error()
            }), EntityTypeEnum) : EntityTypeEnum).TX_shengjiLZ);
            var skillExplosion = instantiate(effectPrefab);
            ...
            var geometry = minion == null || (_minion$children$ = minion.children[0]) == null ? void 0 : _minion$children$.getChildByName("Geometry");

            if (geometry) {
              geometry.active = true;
            }

            var route = this.exitRoute;
            var moveChain = tween(minion);

            this._activeMovingMinions.add(minion); // ➕ 追踪当前出门小兵


            var _loop2 = function _loop2(i) {
              var target = route[i].clone();
              ...
              moveChain = moveChain.call(() => {
                var currentPos = minion.getWorldPosition();
                var dir = new Vec3();
                ...
                if (i === 2) {
                  var bSide = find("ThreeDNode/Map/Fences/Scene1/BSide");
                  var bDoor = bSide == null ? void 0 : bSide.getChildByName("B_Door");

                  if (bDoor) {
                    var doorL = bDoor.getChildByName("Door_Left");
                    var doorR = bDoor.getChildByName("Door_Right");
                    (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
                      error: Error()
              ...
              }).to(Vec3.distance(minion.getWorldPosition(), target) / _this2.moveSpeed, {
        ...
        removeMonsterTarget(node) {
          var index = this._targetMonsters.indexOf(node);
