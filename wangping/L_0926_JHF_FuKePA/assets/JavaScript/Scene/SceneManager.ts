import { _decorator, Component, Prefab, Node, director, find, math, Material, Pool, instantiate, CCFloat, game } from 'cc';
...
export class SceneManager extends Component {
    @property({ type: CCFloat, displayName: "背景音效差值" })
    bgmOffset = 0;
    ...
    @property({ type: CCFloat, displayName: "产出金币倍数" })
    coinYieldMultiplier = 3;

    @property({ type: CCFloat, displayName: "每次恢复的血量" })
    hpRecoveryAmount = 10;

    @property({ type: CCFloat, displayName: "恢复间隔（秒）" })
    hpRecoveryInterval = 1;
    ...
    async start() {
        ...
        DataManager.Instance.gridSystemManager = new GridSystemManager(120);
    ...
    initGame() {
        ...
        DataManager.Instance.monsterConMananger.initMonsters();
        ...
        SimplePoolManager.Instance.registerNodePools([
            ...
            {
                key: EntityTypeEnum.Monster,
                prefab: DataManager.Instance.prefabMap.get(EntityTypeEnum.Monster)!,
                count: 300,
            }
        ])
    ...
    isNodeInsideWallArea(node: Node): boolean {
        ...
        const wallTotal = 4;
        if (!walls || walls.length < wallTotal) {
