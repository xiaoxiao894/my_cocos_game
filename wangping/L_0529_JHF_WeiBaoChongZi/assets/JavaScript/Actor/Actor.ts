...
export class Actor extends Component {
    ...
    private playBezierTransfer(from: Node, to: Node, mode: 'collect' | 'deliver', obj: any) {
        if (!from || !to) return;
        ...
        // 目标堆叠管理器（原来三元是同一结果，直接取即可）
        const stackManager = (to as any)['__stackManager'] as StackManager | undefined;

        // 选择要搬运的 item
        let item: Node | null = null;

        if (obj.isPlot4ArrangePos) {
            // 从 from 的 stackManager 顶部取
            const fromSM = (from as any)['__stackManager'] as StackManager | undefined;
            if (!fromSM) {
                console.warn('from 没有 stackManager，无法取出');
                return;
            }
            const slots = fromSM.getAllOccupiedSlots?.() ?? [];
            for (let i = slots.length - 1; i >= 0; i--) {
                const n = slots[i].assignedNode;
                if (n && n.isValid && (n as any)['__isReady'] === true) {
                    item = n;
                    fromSM.releaseSlot(n);
                    break;
                }
            }
            if (!item) return; // 没有可收集的
        } else {
            // 默认：从 children 末尾挑 __isReady 的
            const arr = from.children;
            for (let i = arr.length - 1; i >= 0; i--) {
                const child = arr[i];
                if ((child as any)['__isReady'] === true) {
                    item = child;
                    break;
                }
            }
        ...
        if (this._activeTransferringItems.has(item)) return; // 简单防重入
        this._activeTransferringItems.add(item);

        // 起点：缓存世界坐标并改父节点
        const startPos = new Vec3();
        item.getWorldPosition(startPos);
        ...
        tween(tParam)
            .to(0.25, { t: 1 }, {
                onUpdate: () => {
                    const t = tParam.t;
                    const u = 1 - t;
                    const x = u * u * startPos.x + 2 * u * t * controlPoint.x + t * t * endPos.x;
                    const y = u * u * startPos.y + 2 * u * t * controlPoint.y + t * t * endPos.y;
                    const z = u * u * startPos.z + 2 * u * t * controlPoint.z + t * t * endPos.z;
                ...
                onComplete: () => {
                    ...
                    const bc = item!.getComponent(BoxCollider);
                    if (bc) bc.enabled = false;

                    // 轻微缩放反馈
                    tween(item!)
                        .to(0.15, { scale: new Vec3(1.2, 1.2, 1.2) }, { easing: 'quadOut' })
                        .to(0.05, { scale: Vec3.ONE }, { easing: 'quadOut' })
                        .start();
                    ...
                    // UI 计分（保持原判断）
                    const n = obj.otherCollider.node.name;
                    if (n === PlotEnum.Plot5 && mode === 'collect') {
                        DataManager.Instance.uiPropertManager.coinCollectProperty();
                    } else if (
                        (n === PlotEnum.Plot6 && mode === 'deliver') ||
                        (n === PlotEnum.Plot7 && mode === 'deliver') ||
                        (n === PlotEnum.Plot8 && mode === 'deliver') ||
                        (n === PlotEnum.Plot9 && mode === 'deliver')
                    ) {
