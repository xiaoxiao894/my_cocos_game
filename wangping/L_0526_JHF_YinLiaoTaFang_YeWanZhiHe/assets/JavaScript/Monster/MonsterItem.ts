...
export class MonsterItem extends Component {
    ...
    public deathAni(isPlayer: boolean = false) {
        ...
        setTimeout(() => {
            this.dissolveEffect.forEach((d: DissolveEffect, i: number) => {
                const dummy = { t: 0 }; // 虚拟对象用于 tween
                tween(dummy)
                    .delay(i * 0.03)
                    .call(() => {
                        if (!d.node?.isValid) return;
    ...
    private attack() {
        ...
        if (this._assignedGuardrail) {
            ...
            if (this._assignedGuardrail.node.name.includes("Door")) {
                //门
                let leftNode = this._assignedGuardrail.node.getChildByPath("Door_Left/men/men");
                let rightNode = this._assignedGuardrail.node.getChildByPath("Door_Right/men/men");
                if (leftNode) {
                    let leftMesh: MeshRenderer = leftNode.getComponent(MeshRenderer);

                    if (leftMesh) {
                        leftMesh.setMaterial(DataManager.Instance.sceneManager.doorMaterials[2], 0);
                        leftMesh.setMaterial(DataManager.Instance.sceneManager.doorMaterials[2], 1);
                        setTimeout(() => {
                    ...
                    if (rightMesh) {
                        rightMesh.setMaterial(DataManager.Instance.sceneManager.doorMaterials[2], 0);
                        rightMesh.setMaterial(DataManager.Instance.sceneManager.doorMaterials[2], 1);
                        setTimeout(() => {
                ...
                if (fenceNode) {
                    ...
                    if (fenceMesh) {
                        fenceMesh.setMaterial(DataManager.Instance.sceneManager.guardrailMaterials[2], 0);
                        fenceMesh.setMaterial(DataManager.Instance.sceneManager.guardrailMaterials[2], 1);
                        setTimeout(() => {
