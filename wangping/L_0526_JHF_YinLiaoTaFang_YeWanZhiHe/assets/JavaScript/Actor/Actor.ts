import { _decorator, Animation, AnimationClip, AnimationState, BoxCollider, builtinResMgr, CCFloat, Collider, Color, Component, director, EffectAsset, find, gfx, ICollisionEvent, instantiate, Label, Mat4, Material, math, MeshRenderer, Node, resources, RigidBody, SkeletalAnimation, Slider, Texture2D, tween, UIOpacity, v3, Vec3, Vec4 } from 'cc';
...
export class Actor extends Component {
    ...
    onTriggerEnter(event: ICollisionEvent) {
        ...
        if (deliveryAreaNames.includes(otherNode.name)) {
    ...
    deliverCoinsFun(deltaTime: number) {
        ...
        const deliveryAreaNames = Object.values(CollisionEntityEnum) as string[];
        if (!deliveryAreaNames.includes(this.otherNode.name)) return;
        ...
        if (otherNode.name == CollisionEntityEnum.DeliveryAreas3 || otherNode.name == CollisionEntityEnum.DeliveryAreas4 ||
            ...
            for (let i = 0; i < sideList.length; i++) {
                ...
                if (node.name.includes("Door")) {
                    ...
                    if (doorLeft && doorLeft.children[0]) {
                    ...
                    tween(node)
                        ...
                        .call(() => {
                            ...
                            if (i === sideList.length - 1) {
                                const plane2 = find("Dixing/Plane002");
                                if (plane2) {
                                    plane2.active = true;
                                    const palne02Ani = plane2.getComponent(Animation);
                                    if (palne02Ani) {
                                        palne02Ani.play("dikuaiCS02")
                                    }
                    ...
                    tween(node)
                        ...
                        .call(() => {
                            if (i === newList.length - 1) {
                                const plane3 = find("Dixing/Plane003");
                                if (plane3) {
                                    plane3.active = true;
                                    const palne03Ani = plane3.getComponent(Animation);
                                    if (palne03Ani) {
                                        palne03Ani.play("dikuaiCS03")
                                    }
