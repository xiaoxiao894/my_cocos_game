...
export class ItemMonsterManager extends Component {
    ...
    injuryAni(isPlayer: boolean, whoHitMe: Node | null) {
        ...
        if (DataManager.Instance.isUnlockArmor) {
            if (DataManager.Instance.curWeaponType == PlayerWeaponTypeEnum.CottonSwab) {
                ...
                goalVector = goalVector.normalize().multiplyScalar(isPlayer ? -DataManager.Instance.sceneManager.knifeRepelDistance + -DataManager.Instance.sceneManager.armorRepelDistance : -this.partnerHitpow);
            } else if (DataManager.Instance.curWeaponType == PlayerWeaponTypeEnum.Flamethrower) {
                goalVector = goalVector.normalize().multiplyScalar(isPlayer ? -DataManager.Instance.sceneManager.spitfireRepelDistance + -DataManager.Instance.sceneManager.armorRepelDistance : -this.partnerHitpow);
            }
        } else {
            if (DataManager.Instance.curWeaponType == PlayerWeaponTypeEnum.CottonSwab) {
                goalVector = goalVector.normalize().multiplyScalar(isPlayer ? -DataManager.Instance.sceneManager.cottonSwabRepelDistance : -this.partnerHitpow);
        ...
        // 显示血条
        if (this.hpNode) this.hpNode.active = true;
        ...
        const isInsideDoor = DataManager.Instance.sceneManager.isNodeInsideWallArea(targetNode);

        if (isInsideDoor) {
            tween(this.node)
                .by(0.15, { position: new Vec3(dis.x, 0, dis.y) })
                .start();
        }
    ...
    update(deltaTime: number) {
        ...
        if (toTargetDist > this.chaseRadius) {
            if (this._assignedGuardrail) {
                this._assignedGuardrail.attackingMonsterCount = Math.max(
                    (this._assignedGuardrail.attackingMonsterCount -= 1),
                    0
