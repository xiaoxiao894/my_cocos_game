...
export class SoundManager{
    ...
    private _meraWorkSource:AudioSource = null;
    private _machineWorkSource:AudioSource = null;
    ...
    public initMeraSource(){
        let meraNode:Node =new Node();
        director.getScene().addChild(meraNode);
        director.addPersistRootNode(meraNode);
        this._meraWorkSource = meraNode.addComponent(AudioSource);

        let workNode:Node =new Node();
        director.getScene().addChild(workNode);
        director.addPersistRootNode(workNode);
        this._machineWorkSource = workNode.addComponent(AudioSource);

    }

    // 播放眉拉工作背景音乐
    public playMeraBGM() {
        const clip = this.audioClips.get("DC_jianpan");
        if (!clip) {
            console.error(`DC_jianpan not found!`);
            return;
        }

        this._meraWorkSource.stop();
        this._meraWorkSource.clip = clip;
        this._meraWorkSource.loop = true;
        this._meraWorkSource.play();
    }
    ...
    // 播放眉拉工作背景音乐
    public playMachineBGM() {
        const clip = this.audioClips.get("DC_qixieyunzhuan");
        if (!clip) {
            console.error(`DC_qixieyunzhuan not found!`);
            return;
        }

        this._machineWorkSource.stop();
        this._machineWorkSource.clip = clip;
        this._machineWorkSource.loop = true;
        this._machineWorkSource.play();
    }

    // 停止背景音乐
    public stopMachineBGM() {
        this._machineWorkSource.stop();
    }
    ...
    public preloadAudioClips() {
        //单独优先加载背景音乐
        ResourceManager.Instance.loadRes("bgm/DC_BGM", AudioClip).then( ( clip) => {
