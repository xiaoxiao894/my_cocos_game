import { Mat4, Vec3,Node, tween } from "cc";
import { App } from "../App";
...
export default class GoldMineController {
    ...
    //钱的尺寸
    private _coinWidth: number = 1.5;
    private _coinHeight: number = 0.25;

    //钱池个数
    private _coinLengthNum: number = 2;
    private _coinWidthNum: number = 2;
    ...
    public addCoin(worldPos?: Vec3): void {
        ...
        coinItem.setScale(coinItem.scale.clone().multiplyScalar(1.2))
        let scale = coinItem.scale.clone().multiplyScalar(1.2)
        tween(coinItem)
        .to(0.15, { scale: coinItem.scale.clone().multiplyScalar(1.4) }, { easing: 'quadOut' })
        .to(0.05, { scale: scale }, { easing: 'quadIn' })
        .start();
