import { _decorator, Animation, Collider, Component, find, ICollisionEvent, ITriggerEvent, Mat4, Node, Quat, tween, v3, Vec3 } from 'cc';
...
export class Player extends Entity {
   ...
   // public attackTargetList: any[] = [];
    ...
    onLoad(): void {
        super.onLoad();
        this.backpack = this.node.getChildByName("pack");
    }
    protected start(): void {
        this.triggerNode.initTrigger(this);
        this.attackAni.on(Animation.EventType.FINISHED,()=>{
    ...
    private collectCoin(){
        ...
        // if(GlobeVariable.handOverArea || GlobeVariable.handOverPhase){
    ...
    private coinFly(coin:Node){
        if(coin){
            SoundManager.inst.playAudio("YX_jinbi");
             // === 贝塞尔飞行动画 ===
          //   GlobeVariable.collectCoin = true;
            ...
            const startRot:Quat = coin.worldRotation.clone(); // 初始角度
            const endRot:Quat = this.backpack.worldRotation;           // 最终角度
            ...
            tween(controller)
                ...
                .call(() => {
                    ...
                    tween(coin)
                        .to(0.15, { scale: new Vec3(4.6, 4.6, 4.6) }, { easing: 'quadOut' })
                        .to(0.05, { scale: new Vec3(4, 4, 4) }, { easing: 'quadOut' })
                        .start();

                    this.coinNum++;
                  //  GlobeVariable.collectCoin = false;
        ...
        // 金币之间的高度间隔
        private coinHeightInterval: number = 0.25;
    /**
     * 校准所有金币的位置，解决堆叠断层问题
    ...
    private playAttackSound(){
        SoundManager.inst.playAudio("YX_gongji");
    }

    private playAttackAni(){
        App.playerController.processAttackTargets();
