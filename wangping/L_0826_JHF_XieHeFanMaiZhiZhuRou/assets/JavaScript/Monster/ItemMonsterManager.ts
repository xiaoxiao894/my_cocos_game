...
export class ItemMonsterManager extends Component {
    ...
    */
    update(deltaTime: number) {
        if (this._isDead || !DataManager.Instance.isStartGame || this._isPlaySkill) return;
    ...
    private tryAttack() {
        if (!this._isAttacking && this._canAttack) {
            ...
            let isSkillActive = this.getReward(1);
            if (this._target.name == "Player" && this.node.name == EntityTypeEnum.MonsterBig) {
                this.changeState(MonsterStateDefine.Skill)
    ...
    attack() {
        // 如果不是攻击护栏，就是攻击人
        if (this._target.name == "Player") {
            if (this.node.name == EntityTypeEnum.MonsterBig) {
                const selfPos2 = this.node.worldPosition;
                const targetPos = this._target.worldPosition;
                const distance = Vec3.distance(selfPos2, targetPos);

                if (distance < this.attackDistance) {
                    DataManager.Instance.playerHealthBarManager.underAttack(1, this.node);
                } else {
    ...
    private changeState(state: string) {
        ...
        if (state === MonsterStateDefine.Idle) {
            ...
            this.skeletalAnimation.once(Animation.EventType.FINISHED, (state: AnimationState) => {
    ...
    // 生成随机技能概率
    getReward(number): boolean {
        let land = Math.random();
