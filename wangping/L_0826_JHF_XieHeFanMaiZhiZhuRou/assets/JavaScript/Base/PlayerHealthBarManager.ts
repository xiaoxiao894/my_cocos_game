import { _decorator, CCFloat, Component, Node, Sprite, tween, Vec3 } from 'cc';
...
export class PlayerHealthBarManager extends Component {
    ...
    // 被攻击了
    underAttack(damage: number = 1, monster: Node) {
        if (DataManager.Instance.isPlayAgain) return;

        // 大蜘蛛攻击玩家
        if (monster.name == EntityTypeEnum.MonsterBig) {
            this.knockUp(DataManager.Instance.player.actor.node, monster);
        }
    ...
    // 玩家被击飞（曲线运动）
    private knockUp(playerNode: Node, monster: Node) {
        const startPos = playerNode.worldPosition.clone();

        // 假设击飞方向 = 从怪物指向玩家的反方向
        const dir = new Vec3();
        Vec3.subtract(dir, startPos, monster.worldPosition);
        dir.normalize();

        // 击飞距离和高度
        const knockDistance = 30;
        const knockHeight = 10;
        const duration = 0.5; // 击飞时间

        const endPos = startPos.clone().add(dir.multiplyScalar(knockDistance));

        // === 使用自定义插值模拟抛物线 ===
        tween({ t: 0 })
            .to(duration, { t: 1 }, {
                onUpdate: (target: any, ratio: number) => {
                    const t = target.t; // 0 ~ 1
                    // 水平位置线性插值
                    const x = startPos.x + (endPos.x - startPos.x) * t;
                    const z = startPos.z + (endPos.z - startPos.z) * t;

                    // y 用抛物线： y = h * (1 - (2t-1)^2)
                    const y = startPos.y + knockHeight * (1 - Math.pow(2 * t - 1, 2));

                    playerNode.setWorldPosition(x, y, z);
                }
            })
            .start();
    }
