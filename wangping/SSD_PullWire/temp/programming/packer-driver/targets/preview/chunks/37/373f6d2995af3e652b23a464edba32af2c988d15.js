    ...
    execute: function () {
      ...
      _export("CloudEffctFade", CloudEffctFade = (_dec = ccclass('CloudEffctFade'), _dec2 = property({
        ...
        initCloud() {
          ...
          var siblings = this.node.children; // 收集所有需要渐变的材质
        ...
        collectMaterials(sibling) {
          var targetNode = sibling.getChildByName("Plane001");
          ...
          var meshRenderer = targetNode.getComponent(MeshRenderer);
        ...
        update(deltaTime) {
          ...
          var progress = Math.min(this._elapsedTime / this.duration, 1); // 根据进度计算当前透明度（从255到0）

          var alpha = Math.floor(255 * (1 - progress));
          var currentColor = new Color(255, 255, 255, alpha); // 更新所有材质的透明度
