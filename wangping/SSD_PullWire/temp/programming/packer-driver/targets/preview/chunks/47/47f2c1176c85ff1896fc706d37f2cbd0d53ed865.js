      ...
      _export("CloudEffct", CloudEffct = (_dec = ccclass('CloudEffct'), _dec2 = property({
        ...
        initMaterials() {
          ...
          siblings.forEach(sibling => {
            var meshNode = sibling.getChildByName("Plane001");
            if (!meshNode) return;
            ...
            var startColor = new Color(255, 255, 255, this.initialOpacity);
        ...
        scheduleFade(material, action) {
          var state = this.materialStates.get(material);
        ...
        startFade(material, action) {
          ...
          var currentColor = material.getProperty(this.colorPropertyName);
          var currentOpacity = currentColor.a; // 检查是否需要执行淡入操作
          ...
          var startOpacity = action === 'fadeIn' ? this.targetOpacity : this.initialOpacity;
          var endOpacity = action === 'fadeIn' ? this.initialOpacity : this.targetOpacity;
          var startColor = new Color(255, 255, 255, startOpacity);
          var endColor = new Color(255, 255, 255, endOpacity); // 创建并存储tween

          var tweenInstance = tween(startColor).to(this.fadeDuration, {
            a: endOpacity
          }, {
            ...
            if (state.pendingAction) {
              var nextAction = state.pendingAction;
        ...
        checkMaterialCompatibility(material) {
          try {
            ...
            // 尝试常见的其他属性名
            var alternativeNames = ['color', 'mainColor', 'diffuseColor'];

            for (var name of alternativeNames) {
              try {
