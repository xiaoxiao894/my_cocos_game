    ...
    execute: function () {
      ...
      _export("PoolManager", PoolManager = class PoolManager {
        ...
        getNode(poolName) {
          ...
          var pool = this._poolDict.get(poolName);

          var prefab = this._prefabDict.get(poolName); // 对象池中有可用节点
        ...
        _expandPool(poolName, expandSize) {
          ...
          var prefab = this._prefabDict.get(poolName);
        ...
        printPoolsStatus() {
          ...
          this._poolDict.forEach((pool, name) => {
            var prefab = this._prefabDict.get(name);
