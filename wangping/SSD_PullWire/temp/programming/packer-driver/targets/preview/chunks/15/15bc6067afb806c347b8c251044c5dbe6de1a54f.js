    ...
    execute: function () {
      ...
      _export("default", TransformPositionUtil = class TransformPositionUtil {
        ...
        static getDepthRatio(camera, worldPos) {
          ...
          var depth = -localPos.z; // 视图空间中的深度值
          // 计算标准化深度 (0到1之间)
        ...
        static getProjectionRatio(a, b, c) {
          // 计算向量AB和AC
          var ab = b.subtract(a);
          var ac = c.subtract(a); // 计算AB长度的平方

          var abLengthSq = ab.lengthSqr(); // 如果AB长度接近0，返回0避免除零错误
          ...
          var ratio = Vec2.dot(ac, ab) / abLengthSq;
          return ratio;
        }
        /**
         * 获取插件位置 (主方法)
        ...
        static getPlugPos(pos) {
          ...
          var closestPoint = this.getClosestPointOnRay(ray, lastPos); // 2. 应用高度曲线
        ...
        static getClosestPointOnRay(ray, targetPoint) {
          var rayDirection = ray.d.normalize();
          var rayToPoint = Vec3.subtract(new Vec3(), targetPoint, ray.o);
          var projection = Vec3.dot(rayToPoint, rayDirection);
        ...
        static calculateHeight(x, z) {
          ...
          var normalizedDistance = Math.min(distance, 10); // 二次曲线平滑过渡：原点高度10，距离10时高度3
        ...
        static applyBoundaryLimits(pos) {
          // 移动边界限制
          var moveBoundary = 10.0;
          var len = Math.sqrt(pos.x * pos.x + pos.z * pos.z);

          if (len > moveBoundary) {
            var factor = moveBoundary / len;
            pos.x *= factor;
            pos.z *= factor;
          }
        }
        ...
        static applySmoothing(lastPos, targetPos) {
          // 平滑系数 (0.1-1.0，越小越平滑)
          var smoothFactor = 0.3; // 最大移动距离 (防止瞬间跳跃)

          var maxMoveDistance = 5.0; // 计算实际移动距离

          var moveDistance = Vec3.distance(lastPos, targetPos); // 动态调整平滑系数 (移动越快越不平滑)

          var dynamicSmooth = Math.min(smoothFactor, 1 / (moveDistance + 0.1)); // 限制最大移动距离
