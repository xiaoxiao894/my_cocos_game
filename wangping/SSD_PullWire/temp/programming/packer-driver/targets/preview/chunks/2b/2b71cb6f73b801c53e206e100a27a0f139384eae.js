      ...
      _export("FruitScript", FruitScript = (_dec = ccclass('FruitScript'), _dec2 = property(Material), _dec3 = property(Material), _dec4 = property({
        ...
        cacheMeshRenderers() {
          ...
          this.fruitNode.children.forEach(child => {
            var fanqieNode = child.getChildByName("fanqie");

            if (fanqieNode) {
              var meshRenderer = fanqieNode.getComponent(MeshRenderer);
        ...
        setFruitState(state, isFirst) {
          ...
          var targetMaterial = state === 'red' ? this.materialRed : this.materialGreen;
        ...
        startBounce() {
          ...
          var startPos = new Vec3(this.fruitNode.position.x, this.fruitNode.position.y, this.fruitNode.position.z); // 计算弹跳的最高点

          var peakPos = new Vec3(startPos.x, startPos.y + this.bounceHeight, startPos.z); // 计算下落时超过起始点的位置

          var overshootPos = new Vec3(startPos.x, startPos.y - this.overshootDistance, startPos.z); // 对整个 fruitNode 应用弹跳动画
