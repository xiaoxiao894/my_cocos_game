      ...
      _export("RepoTouchComponent", RepoTouchComponent = (_dec = ccclass('RepoTouchComponent'), _dec2 = property(Node), _dec3 = property(CCInteger), _dec(_class = (_class2 = class RepoTouchComponent extends Component {
        ...
        onTouchStart(event) {
          ...
          if (PhysicsSystem.instance.raycastClosest(ray, mask, maxDistance, queryTrigger)) {
            ...
            var plug = raycastClosestResult.collider;

            if (plug) {
              var plugItem = plug.node.getComponent(_crd && PlugItem === void 0 ? (_reportPossibleCrUseOfPlugItem({
                error: Error()
              }), PlugItem) : PlugItem);
        ...
        moveCamera(event) {
          ...
          var moveDir = this.screenToWorldDirection(deltaPos, cameraMain.node); // 2. 保持Y轴不变

          moveDir.y = 0;
          moveDir.normalize(); // 3. 计算移动距离(应用灵敏度)

          var moveDistance = deltaPos.length() * this.sensitivity; // 4. 更新摄像机位置

          var cameraPos = cameraMain.node.getWorldPosition().clone();
          Vec3.scaleAndAdd(cameraPos, cameraPos, moveDir, moveDistance);
          var cameraLimit = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
        ...
        cameraFoollowPlug(event) {
          ...
          var moveDir = this.screenToWorldDirection(deltaPos, cameraMain.node);
          moveDir.y = 0;
          moveDir.normalize();
          ...
          var minMoveDistance = 0.5;
          var maxMoveDistance = 1; // 限制最大移动距离，防止飞出

          var moveDistance = deltaPos.length() * this.sensitivity * 1;
        ...
        movePlug(event) {
          ...
          var endWorldPos = (_crd && MathUtil === void 0 ? (_reportPossibleCrUseOfMathUtil({
            error: Error()
          ...
          var nowPlugPos = (_crd && MathUtil === void 0 ? (_reportPossibleCrUseOfMathUtil({
            error: Error()
          }), MathUtil) : MathUtil).worldToLocal((_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          ...
          var distance = Vec3.distance(nowPlugPos, endWorldPos); //    let endWorldPos: Vec3 = DataManager.Instance.nowSocket.worldPosition;
          ...
          if (distance < (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            ...
            if (this._nowSocketIndex == 2) {
              //野兽
              var cameraPos = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
                error: Error()
              ...
              //野兽 油桶
              var _cameraPos = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
                error: Error()
              }), DataManager) : DataManager).Instance.mainCamera.node.getWorldPosition();

              if (_cameraPos.x > cameraLimit.y + 5) {
                // 使用 tween 实现平滑移动到 x=14，持续时间0.5秒
                tween((_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
                  error: Error()
                }), DataManager) : DataManager).Instance.mainCamera.node).to(0.8, {
                  worldPosition: new Vec3(cameraLimit.y + 5, _cameraPos.y, _cameraPos.z)
                }).start();
              }
            } else if (this._nowSocketIndex == 1) {
              //矿场
              var _cameraPos2 = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
                error: Error()
              }), DataManager) : DataManager).Instance.mainCamera.node.getWorldPosition();

              if (_cameraPos2.x < cameraLimit.x) {
                // 使用 tween 实现平滑移动到 x=14，持续时间0.5秒
                tween((_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
                  error: Error()
                }), DataManager) : DataManager).Instance.mainCamera.node).to(0.8, {
                  worldPosition: new Vec3(cameraLimit.x - 1, _cameraPos2.y, _cameraPos2.z)
                }).start();
              }
            } else if (this._nowSocketIndex == 0) {
              //农场
              var _cameraPos3 = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
                error: Error()
              }), DataManager) : DataManager).Instance.mainCamera.node.getWorldPosition();

              if (_cameraPos3.x > cameraLimit.y + 5) {
                // 使用 tween 实现平滑移动到 x=14，持续时间0.5秒
                tween((_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
                  error: Error()
                }), DataManager) : DataManager).Instance.mainCamera.node).to(0.8, {
                  worldPosition: new Vec3(cameraLimit.y + 5, _cameraPos3.y, _cameraPos3.z)
                }).start();
              }
            }
          ...
          var plot = (_crd && RopeUtils === void 0 ? (_reportPossibleCrUseOfRopeUtils({
            error: Error()
          ...
          var touchPos = event.getLocation(); // const cameraMain:Camera = DataManager.Instance.mainCamera.camera;
          ...
          var eulerAngles = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          ...
          var moveLen = Vec2.distance(touchPos, this._lastTouchPos); // 计算旋转步长（转换为弧度）

          var maxAngle = this._plugRotateSpeed * moveLen * Math.PI / 180; // 使用球面线性插值(Slerp)

          var currentRot = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
        ...
        onTouchEnd(event) {
          ...
          if (this._movingPlug) {
            ...
            var endPos = this._plugStartPos.clone();
            ...
            tween((_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              ...
              var plugItem = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
                error: Error()
        ...
        plugLinkSocket() {
          ...
          var nowPlug = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
        ...
        getDepthByPos(pos) {
          ...
          var startWorldPos = this._plugStartPos.clone();

          var endWorldPos = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          ...
          if (startWorldPos && endWorldPos) {
            ...
            var startDepth = (_crd && TransformPositionUtil === void 0 ? (_reportPossibleCrUseOfTransformPositionUtil({
              error: Error()
            }), TransformPositionUtil) : TransformPositionUtil).getDepthRatio(cameraMain, startWorldPos);
            var endDepth = (_crd && TransformPositionUtil === void 0 ? (_reportPossibleCrUseOfTransformPositionUtil({
              error: Error()
            }), TransformPositionUtil) : TransformPositionUtil).getDepthRatio(cameraMain, endWorldPos); //屏幕坐标

            var startSceenPos = cameraMain.worldToScreen(startWorldPos);
            var endSceenPos = cameraMain.worldToScreen(endWorldPos); //插头距离比值

            var ropePosRatio = (_crd && TransformPositionUtil === void 0 ? (_reportPossibleCrUseOfTransformPositionUtil({
              error: Error()
        ...
        getPlugPos(pos) {
          var camera = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          }), DataManager) : DataManager).Instance.mainCamera.camera;
          var ray = camera.screenPointToRay(pos.x, pos.y);
          var lastPos = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          }), DataManager) : DataManager).Instance.nowPlug.worldPosition.clone(); // 计算射线上距离lastPos最近的点

          var closestPoint = this.getClosestPointOnRay(ray, lastPos); // 应用高度计算
        ...
        getClosestPointOnRay(ray, targetPoint) {
          // 计算射线方向向量
          var rayDirection = ray.d.normalize(); // 计算从射线起点到目标点的向量

          var rayToPoint = Vec3.subtract(new Vec3(), targetPoint, ray.o); // 计算投影长度

          var projection = Vec3.dot(rayToPoint, rayDirection); // 如果投影为负，说明最近点是射线起点
        ...
        calculateHeight(x, z) {
          ...
          var normalizedDistance = Math.min(distance, 15); //console.log("distance",distance,"normalizedDistance",normalizedDistance);
          // 使用平滑的曲线过渡

          var height = 4 + 6 * (1 - normalizedDistance / 15) * (1 - normalizedDistance / 15);
          return height;
        } //检查范围内插座、更新目标插头
