    ...
    execute: function () {
      ...
      _export("RopeManager", RopeManager = (_dec = ccclass('RopeManager'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(Node), _dec6 = property(CCInteger), _dec7 = property(Node), _dec8 = property(Node), _dec9 = property(Node), _dec10 = property(Node), _dec(_class = (_class2 = class RopeManager extends Component {
        ...
        start() {
          ...
          for (var i = 0; i < this._startRopeNum; i++) {
            this.creatPope(i);
          }

          for (var _i = this._startRopeNum; _i < this.headNodes.length; _i++) {
            this.headNodes[_i].active = false;
            this.endNodes[_i].active = false;
          }
        }
        ...
        creatPope(index) {
          ...
          var newParent = instantiate(this.completeRope);
          var item = newParent.getComponent(_crd && CompleteRopeItem === void 0 ? (_reportPossibleCrUseOfCompleteRopeItem({
            error: Error()
        ...
        creatLeftRopes() {
          ...
          var _loop = function _loop(i) {
            if ((_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              ...
              _this.scheduleOnce(() => {
                _this.headNodes[i].setPosition(new Vec3(-0.837, 0, 3));

                _this.endNodes[i].setPosition(new Vec3(-1.305, 0, 1.8)); //  this.headNodes[i].setRotation();
                ...
                _this.endNodes[i].setRotation(rot); // this.endNodes[i].setRotation(this.endNodes[1].rotation);
            ...
            _this.scheduleOnce(() => {
              _this.schedule(() => {
                _this.headNodes[i].active = true;
                _this.endNodes[i].active = true;
              }, 1);
          ...
          for (var i = this._startRopeNum; i < this.headNodes.length; i++) {
            _loop(i);
          }
        ...
        onPlugStateUpdate() {
          ...
          for (var i = 0; i < this.endNodes.length; i++) {
            var plug = this.endNodes[i].getComponent(_crd && PlugItem === void 0 ? (_reportPossibleCrUseOfPlugItem({
              error: Error()
            }), PlugItem) : PlugItem);
          ...
          if (state1Index !== -1) {
            var socketIndex = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              error: Error()
        ...
        */
        ...
        unbatchAllAndMoveHeads() {
          for (var i = 0; i < this._ropes.length; i++) {
            var ropeNode = this._ropes[i];
            var ropeComp = ropeNode.getComponent(_crd && CompleteRopeItem === void 0 ? (_reportPossibleCrUseOfCompleteRopeItem({
              error: Error()
            }), CompleteRopeItem) : CompleteRopeItem);
          ...
          for (var _i2 = 0; _i2 < 5; _i2++) {
            var pos1 = this.headNodes[_i2].getPosition();

            this.headNodes[_i2].setPosition(new Vec3(pos1.x - 0.5, pos1.y + 1.5, pos1.z)); // let pos: Vec3 = this.headNodes[i].getWorldPosition().clone();
        ...
        continueFunc() {
          for (var index = this._nowRopeIndex; index < this.endNodes.length; index++) {
        ...
        connectLeftOneRope() {
          ...
          var plug = this.endNodes[this._nowRopeIndex].getComponent(_crd && PlugItem === void 0 ? (_reportPossibleCrUseOfPlugItem({
            error: Error()
          }), PlugItem) : PlugItem);
          ...
          if (plug && plug.state == 0) {
            ...
            var plugTartetRot = new Quat();
            Quat.fromEuler(plugTartetRot, eulerAngles.x, eulerAngles.y, eulerAngles.z);
            var endPos = this.getEndPos((_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              error: Error()
            }), DataManager) : DataManager).Instance.socketNodes[socketIndex]);
            var distance = Vec3.distance(this.endNodes[this._nowRopeIndex].worldPosition, endPos);
            var t = distance / (this.leftRopeMoveSpeed * 1000) + 1.3;
            var cloudHide = false;
            var tweenHandel = tween(this.endNodes[this._nowRopeIndex]).to(t, {
              ...
              onUpdate(target, ratio) {
                //检测距离靠近直接走插入动画
                var nowDistan = Vec3.distance(self.endNodes[self._nowRopeIndex].worldPosition, endPos);
        ...
        */
        ...
        plugLinkSocket(socketIndex, plugNode) {
          var dis = 2;
          var moveDis = this.getBackwardMoveVector((_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
        ...
        getEndPos(socketNode) {
          var endPos = (_crd && MathUtil === void 0 ? (_reportPossibleCrUseOfMathUtil({
            error: Error()
