      ...
      _export("RopeGeneratorNew", RopeGeneratorNew = (_dec = ccclass('RopeGeneratorNew'), _dec2 = property(Node), _dec(_class = (_class2 = class RopeGeneratorNew extends Component {
        ...
        updateRopeVisuals() {
          ...
          for (let i = 0; i < this.rope.length - 1; i++) {
            ...
            let rotation = new Quat();
            Quat.rotationTo(rotation, new Vec3(0, 1, 0), sampleDir);
            currentNode.setWorldRotation(rotation); // 使用完整长度的圆柱体表示绳子段

            let scale = new Vec3(this._diameter, length, this._diameter);
            currentNode.setWorldScale(scale); // 将节点放在两点之间的中心
        ...
        createRope(nodes, staticTar, plugTar) {
          ...
          let prevBody = headRb._body.impl; // 绑定头节点到插头

          const plugBody = plugRb._body.impl;
          ...
          const headDistanceConstraint = new CANNON.DistanceConstraint(prevBody, plugBody, -0.1, 100 // 降低强度以增加柔软度
          );
          this.cannonWorld.addConstraint(headDistanceConstraint);
          this.joints.push(headDistanceConstraint);
          const curvePoints = (_crd && MathUtil === void 0 ? (_reportPossibleCrUseOfMathUtil({
            error: Error()
          ...
          for (let i = 1; i < nodes; i++) {
            ...
            const currBody = newRb._body.impl; //const distance = Vec3.distance(prevNode.worldPosition,newNode.worldPosition);
            // 使用距离约束

            const distanceConstraint = new CANNON.DistanceConstraint(prevBody, currBody, 0, 100 // 降低强度以增加柔软度
            );
            ...
            if (i > 1) {
              ...
              const hingeConstraint = new CANNON.ConeTwistConstraint(prevBody, currBody, {
          ...
          const tailBody = tailRb._body.impl;
          const staticBody = staticRb._body.impl;
        ...
        smoothRopeMovement() {
          ...
          for (let i = 0; i < this.rope.length; i++) {
            ...
            if (!rb) continue;
            const body = rb._body.impl; // 降低角速度
            ...
            if (i > 0 && i < this.rope.length - 1) {
              ...
              const nextPos = nextNode.getWorldPosition(); // 计算平均位置

              const avgPos = new Vec3((prevPos.x + nextPos.x) * 0.5, (prevPos.y + nextPos.y) * 0.5, (prevPos.z + nextPos.z) * 0.5); // 轻微地向平均位置移动

              const currPos = node.getWorldPosition();
              const newPos = new Vec3(currPos.x * 0.95 + avgPos.x * 0.05, currPos.y * 0.95 + avgPos.y * 0.05, currPos.z * 0.95 + avgPos.z * 0.05); // 应用新位置
          ...
          if (this.head) {
            ...
            if (headRb) {
              const headBody = headRb._body.impl;
