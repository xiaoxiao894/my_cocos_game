      ...
      _export("PalingAnimation", PalingAnimation = (_dec = ccclass('PalingAnimation'), _dec2 = property({
        ...
        startAnimation() {
          return new Promise(resolve => {
            ...
            const nodeToAnimate = this.targetNode || this.node; // 确保节点是激活状态

            if (!nodeToAnimate.active) {
              nodeToAnimate.active = true;
            }

            const siblings = nodeToAnimate.children; // 如果没有子节点，直接resolve
        ...
        startMoveDown() {
          const originalPosition = new Vec3(this.node.position.x, this.node.position.y - this.startOffsetY, this.node.position.z);
          const tweenInstance = tween(this.node).by(this.duration, {
        ...
        startBounce() {
          ...
          const node = this.targetNode || this.node; // 确保节点处于激活状态

          node.active = true; // 获取当前位置作为起始点

          const startPos = new Vec3(node.position.x, node.position.y, node.position.z); // 计算弹跳的最高点

          const peakPos = new Vec3(startPos.x, startPos.y + this.bounceHeight, startPos.z); // 计算下落时超过起始点的位置

          const overshootPos = new Vec3(startPos.x, startPos.y - this.overshootDistance, startPos.z); // 分阶段设置动画
        ...
        playAnimationSequentially(siblings, index, resolve) {
          ...
          const tweenInstance = tween(sibling).to(this.duration, {
            position: originalPosition
          }, {
            easing: "quadIn",
            //"backOut",
