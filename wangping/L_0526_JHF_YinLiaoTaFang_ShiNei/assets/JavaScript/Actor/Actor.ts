...
import { AreaEnum, CollisionEntityEnum, EntityTypeEnum, GamePlayNameEnum, PathEnum, PlayerWeaponTypeEnum } from '../Enum/Index';
...
//  叉子
//  水果刀      攻击范围： 360，    攻击距离： 10
//  喷火枪                60度     攻击距离:  15        击退范围  
...
export class Actor extends Component {
    ...
    start() {
        ...
        if (JackParent) {
            ...
            if (jack) {
                const txAttack = JackParent.getChildByName("TX_attack_SGD");
    ...
    lateUpdate(dt: number) {
        ...
        if (JackParent) {
            const jack = JackParent.getChildByName("JackA-ZJ");
            if (jack) {
                const txAttack = jack.getChildByName("TX_attack_SGD");
                if (txAttack) {
    ...
    doMove() {
        let speed = this.linearSpeed * this.destForward.length() * 18;
    ...
    private deliverItemToTarget(targetNode: Node): boolean {
        ...
        if (deliveryNumSprite) {
            ...
            tween(deliveryNumSprite)
                .to(0.1, { scale: new Vec3(0.04, 0.04, 0.04) })
                .to(0.1, { scale: new Vec3(0.03, 0.03, 0.03) })
                .start();
        }
    ...
    onDeliveryComplete(otherNode) {
        ...
        if ((otherNode.name == CollisionEntityEnum.DeliveryAreas11 && DataManager.Instance.curWeaponIdx == 1) ||
            otherNode.name != CollisionEntityEnum.DeliveryAreas11) {
            tween(uiOpacity)
                .to(0.3, { opacity: 0 })
                .call(() => {
            ...
            this.updateWeaponUnlockValue(otherNode);
        }
    ...
    unlockNewSpecies(otherNode) {
        ...
        if (otherNode.name == CollisionEntityEnum.DeliveryAreas6) {             // 解锁场景1
            const targetData = DataManager.Instance.guideTargetList.find(item => {
                return item.name == CollisionEntityEnum.DeliveryAreas11;
            })

            if (targetData) {
                targetData.isFind = true;
                targetData.isDisplay = true;

                if (!targetData.node.active) {
                    targetData.node.active = true;

                    const plot = targetData.node.getChildByName("Plot");
                    if (plot) {
                        plot.active = true;
                    }
                }
            ...
            // 提升怪物数量
            DataManager.Instance.monsterAllNum = 100;
            DataManager.Instance.monsterBornTimeLimit = 1;
            ...
            for (let i = 0; i < sideList.length; i++) {
                ...
                if (node.name.includes("Door")) {
                    const doorLeft = node.getChildByName("Door_Right")
                    ...
                    const doorRight = node.getChildByName("Door_Right")
                    ...
                    if (meshRender) meshRender.shadowCastingMode = 1;
                }
            ...
            otherNode.name == CollisionEntityEnum.DeliveryAreas2 || otherNode.name == CollisionEntityEnum.DeliveryAreas3 ||
            otherNode.name == CollisionEntityEnum.DeliveryAreas4 || otherNode.name == CollisionEntityEnum.DeliveryAreas5 ||
            otherNode.name == CollisionEntityEnum.DeliveryAreas8) {
            const buildingCon = otherNode.getChildByName("BuildingCon");
            if (buildingCon) {
                ...
                if (minion) {
                    ...
                    if (minionManager) {
                        ...
                        this.triggerUpgradeParticles(worldPos);
        ...
        } else if (otherNode.name == CollisionEntityEnum.DeliveryAreas11) {                  // 变换武器
            this.updatePlayerWeapon(otherNode);
        }
    }

    // 更新角色武器
    updatePlayerWeapon(otherNode) {
        const plot = otherNode.getChildByName("Plot");
        const worldPos = otherNode.worldPosition;
        this.triggerUpgradeParticles(worldPos);

        DataManager.Instance.curWeaponType = PlayerWeaponTypeEnum.Flamethrower;
        DataManager.Instance.curWeaponIdx = 2;
        DataManager.Instance.playerWeaponDamage = 3;

        if (this.knifeNode) this.knifeNode.active = false;
        if (this.flamethrowerNode) this.flamethrowerNode.active = true;

        this.scheduleOnce(() => {
            const playerManager = this.node.getComponent(PlayerManager);
            if (playerManager) {
                playerManager.hideAllAnimation();
            }
        }, .1)
        DataManager.Instance.playerAction = true;
    ...
    // 升级粒子特效
    triggerUpgradeParticles(worldPos) {
    ...
    // 更改解锁武器数值
    updateWeaponUnlockValue(otherNode: Node) {
        if (!otherNode || !otherNode.isValid) return;
        (otherNode as any)._hasDeliveredZero = false;
        ...
        if (scene1RSide.active) {
            otherNode.active = false;
            if (plot) plot.active = false;
        }

        const deliveryNumNode = plot.getChildByName("DeliveryNum");
        const dkLv = plot?.getChildByName("DK_LV");
        const dkLvSpr = dkLv?.getComponent(Sprite);
        const baseFigure = plot.getChildByName("BaseFigure");
        const baseFigure1 = plot.getChildByName("BaseFigure-001");

        DataManager.Instance.playerAction = true;

        if (baseFigure) baseFigure.active = false;
        if (baseFigure1) baseFigure1.active = true;

        if (this.cottonSwabNode) this.cottonSwabNode.active = false;
        if (this.knifeNode) this.knifeNode.active = true;

        const worldPos = otherNode.worldPosition;
        this.triggerUpgradeParticles(worldPos);

        DataManager.Instance.curWeaponIdx = 1;

        DataManager.Instance.curWeaponType = PlayerWeaponTypeEnum.Knife;
        DataManager.Instance.curWeaponIdx = 1;
        DataManager.Instance.playerWeaponDamage = 2;
        ...
        if (deliveryNumNode) {
            const deliveryNumCom = deliveryNumNode.getComponent(Label);
            if (deliveryNumCom) {
                const newValue = 50;
                deliveryNumCom.string = `${newValue}`;

                // 同步到 deliveryCountMap
                this.deliveryCountMap.set(otherNode, newValue);

                const plotData = DataManager.Instance.landParcelRules.find(item => {
                    return item.name == CollisionEntityEnum.DeliveryAreas11;
                })

                if (plotData) {
                    plotData.iconNumber = newValue;
                }

                if (dkLvSpr) dkLvSpr.fillRange = 0;

                // 数字缩放动画
                tween(deliveryNumNode)
                    .to(0.1, { scale: new Vec3(0.04, 0.04, 0.04) })
                    .to(0.1, { scale: new Vec3(0.03, 0.03, 0.03) })
                    .start();
            }
