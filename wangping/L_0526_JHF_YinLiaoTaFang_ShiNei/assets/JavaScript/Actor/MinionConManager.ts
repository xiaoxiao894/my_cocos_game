import { _decorator, Component, instantiate, Node, Vec3, tween, find, director, Animation, Camera, AnimationClip, SkeletalAnimation } from 'cc';
...
import { cloneAnimationClipSafe } from '../Animation/AnimationClipClone';
...
export class MinionConManager extends Component {
    ...
    // 出门路径
    exitRoute = [new Vec3(-0.657, 0, 25), new Vec3(8.411, 0, 25), new Vec3(8.411, 0, 35)];
    ...
    // 填充队列
    private isBubbleShown = false; // 默认未执行
    checkQueueAndRefill() {
        if (!this.node) return;
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            const childMinion = minion.getChildByName("Minion");
            const minionManager = childMinion?.children[0].getComponent(MinionManager);
            ...
            // 开始移动时切换为 Walk
            minionManager.changState(MinionStateEnum.initWalk);
        ...
        if (children.length < this.queuePosition.length && this._spawnTimer >= this.spawnCooldown) {
            ...
            tween(minion)
                ...
                .call(() => {
                    ...
                    if (children.length > 0 && this.isBubbleShown) {
                        const bubble = children[0].getChildByName("Bubble");
        ...
        if (DataManager.Instance.isGenerateWeapons && children.length > 0 && !this.isBubbleShown) {
            const bubble = children[0].getChildByName("Bubble");
            if (bubble) bubble.active = true;
    ...
    goOutMinion() {
        ...
        tween(controller)
            ...
            .call(() => {
                ...
                const bubble = minion.getChildByName("Bubble");
                const bubbleManager = bubble.getComponent(BubbleManager);
                if (bubbleManager) {
                    bubbleManager.bubbleAni();
                }
                ...
                skillExplosion.setScale(0.5, 0.5, 0.5)
                skillExplosion.setWorldPosition(new Vec3(worldPos.x, worldPos.y, worldPos.z + 1));
                ...
                if (minionManager) {
                    ...
                    minionManager.sgdNode.active = true;
                }
    ...
    soldiersChasingMonsters(minion) {
        ...
        const minionChild = minion.getChildByName("Minion").getChildByName("JackA-ZJ");
        const minionManager = minionChild.getComponent(MinionManager);
