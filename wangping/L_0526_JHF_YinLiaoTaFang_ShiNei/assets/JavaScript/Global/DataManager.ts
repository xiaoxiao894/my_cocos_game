...
import { CollisionEntityEnum, EntityTypeEnum, PlayerWeaponTypeEnum } from '../Enum/Index';
...
export class DataManager extends Singleton {
    ...
    landParcelRules = [
        ...
        {
            name: CollisionEntityEnum.DeliveryAreas11,       // 地块名称
            iconNumber: 15,                                  // 需要金币数量                   解锁场景2，  结束
            isUnlockPartners: false                         // 是否解锁伙伴
        },
    ]
    ...
    doors = [];                                                                 // 四个门    
    isGenerateWeapons = false;                                                  // 是否开始生成武器            
    isConveyorBeltUnlocking = false;                                            // 传送带是否解锁
    curWeaponType: PlayerWeaponTypeEnum = PlayerWeaponTypeEnum.CottonSwab;      // 当前武器
    curWeaponIdx = 0;                                                           // 0的时候是刀，1的时候是喷火器
    playerWeaponDamage = 1;                                                     // 武器造成的伤害
