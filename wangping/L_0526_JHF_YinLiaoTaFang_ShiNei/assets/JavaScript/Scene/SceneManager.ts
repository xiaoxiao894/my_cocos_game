import { _decorator, Component, Prefab, Node, director, find, math, Material, Pool, instantiate, Vec3, CCFloat } from 'cc';
...
export class SceneManager extends Component {
    ...
    @property({ type: CCFloat, tooltip: "叉子攻击距离" })
    forkAttackDistance = 5;

    @property({ type: CCFloat, tooltip: "叉子攻击角度" })
    forkAttackAngle = 360;

    @property({ type: CCFloat, tooltip: "叉子击退距离" })
    forksRepelDistance = 3;

    @property({ type: CCFloat, tooltip: "小刀攻击距离" })
    knifeAttackDistance = 5;

    @property({ type: CCFloat, tooltip: "小刀攻击角度" })
    knifeAttackAngle = 360;

    @property({ type: CCFloat, tooltip: "小刀击退距离" })
    knifeRepelDistance = 3;

    @property({ type: CCFloat, tooltip: "喷火器攻击距离" })
    spitfireAttackDistance = 5;

    @property({ type: CCFloat, tooltip: "喷火器攻击角度" })
    spitfireAttacAngle = 180;

    @property({ type: CCFloat, tooltip: "喷火器击退距离" })
    ...
    updateGuidanceData() {
        ...
        if (deliveryAreas) {
            ...
            DataManager.Instance.guideTargetList.splice(4, 0, ...DataManager.Instance.guideTargetList.splice(-1));
        }
    }
    ...
    collectGuardrails(scene) {
        let sceneChildrenLength = scene.children.length;
        if (scene.name == "Scene1") sceneChildrenLength = scene.children.length - 1;

        for (let i = 0; i < sceneChildrenLength; i++) {
