      ...
      _export("WorldMap", WorldMap = (_dec = ccclass('WorldMap'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(Node), _dec6 = property(Node), _dec7 = property(Node), _dec8 = property(_crd && GroundEffct === void 0 ? (_reportPossibleCrUseOfGroundEffct({
        ...
        handoverNo() {
          this.treeParentNode.children.forEach(item => {
            if (item.active) {
              let enemy = item.getComponent(_crd && EnemyTree === void 0 ? (_reportPossibleCrUseOfEnemyTree({
                error: Error()
              }), EnemyTree) : EnemyTree);
        ...
        updateHandOverProgress() {
          let sp = this.greenScript.getComponent(Sprite);
          let curNum = sp.node.getParent().getChildByName("Label").getComponent(Label);
          let maxNum = sp.node.getParent().getChildByName("Label-001").getComponent(Label); // 确保数值在有效范围内

          const current = Math.max(0, Math.min((_crd && Global === void 0 ? (_reportPossibleCrUseOfGlobal({
            error: Error()
          }), Global) : Global).treeHandOverNum, (_crd && Global === void 0 ? (_reportPossibleCrUseOfGlobal({
            error: Error()
          }), Global) : Global).treeHandOverNumLimit));
          const max = Math.max(1, (_crd && Global === void 0 ? (_reportPossibleCrUseOfGlobal({
            error: Error()
          ...
          const percentage = current / max; // 更新进度条
        ...
        cornCutCallback(selfNode) {
          for (let i = 1; i < (_crd && Global === void 0 ? (_reportPossibleCrUseOfGlobal({
            error: Error()
        ...
        clickEnemyAttack(arrowEnemynode) {
          console.log("点击攻击敌人");
          let enemy = arrowEnemynode.getComponent(_crd && enemyCharacter === void 0 ? (_reportPossibleCrUseOfenemyCharacter({
            error: Error()
        ...
        enemyHandeOverAdd() {
          let sp = this.greenScript_1.getComponent(Sprite); // 确保数值在有效范围内
...
//# sourceMappingURL=3e866a0f52d22cd218b6df86f7e3c2fc94acf0fb.js.map