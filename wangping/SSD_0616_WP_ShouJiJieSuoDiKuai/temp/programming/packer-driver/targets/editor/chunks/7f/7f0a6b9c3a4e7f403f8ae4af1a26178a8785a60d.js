  ...
  return {
    ...
    execute: function () {
      ...
      _export("PlayerController", PlayerController = (_dec = ccclass('PlayerController'), _dec(_class = (_class2 = class PlayerController extends Component {
         ...
         * @param parentNode 
         */
        async initCharacters(parentNode, birthNodesPos, endNodesPos, cornNodesPos, enemyHandOverPos, allCharacterPos, enemySelfLanPos) {
          if (birthNodesPos) {
          ...
          try {
            ...
            for (let i = 0; i < birthNodesPos.length; i++) {
              ...
              const character = characterNode.getComponent(_crd && Character === void 0 ? (_reportPossibleCrUseOfCharacter({
                error: Error()
              }), Character) : Character);
              ...
              character.entitySetPosition(this.birthNodesPos[i].position); // 添加到角色列表和场景

              this.characterList.push(character);
              this.parentNode.addChild(characterNode); // character.idle(character);
              // 触发角色初始化动画
        ...
        squenceMove(target, behaviourType) {
          ...
          for (let i = 0; i < this.characterList.length; i++) {
            ...
            const character = this.characterList[characterIndex]; // 找到未在寻找目标的角色
        ...
        squenceHandOver(target, behaviourType) {
          ...
          for (let i = 0; i < this.characterList.length; i++) {
            //const characterIndex = (initialIndex + i) % this.characterList.length;
            const character = this.characterList[i]; // 找到未在寻找目标的角色//!character.getFindTarget() &&
            ...
            if (character.woodNum > 0) {
              ...
              character.strtMoveHandOver(); // }, 0.1)
        ...
        moveFindEnemyLandPos(enemyParent) {
          ...
          for (let i = 0; i < this.characterList.length; i++) {
            ...
            let enemyScrpt = enemy.getComponent(_crd && enemyCharacter === void 0 ? (_reportPossibleCrUseOfenemyCharacter({
              error: Error()
        ...
        resetState() {
          for (let i = 0; i < this.characterList.length; i++) {
            const character = this.characterList[i]; // 找到未在寻找目标的角色
        ...
        cutCornController(selfNode, groundEffect) {
          ...
          const id = (_arrowIds$selfNode$na = arrowIds[selfNode.name]) != null ? _arrowIds$selfNode$na : -1; // 默认值设为-1表示未匹配
        ...
        GameHandOverComplate() {
          ...
          for (let i = 0; i < this.characterListPos.length; i++) {
            ...
            }, i * 0.2); // const character =  this.parentNode.children[i].getComponent(Character)
            // character.move()
          }
        }

        createCharacter(prefab1) {
          let prefab = prefab1; // 初始化角色
...
//# sourceMappingURL=7f0a6b9c3a4e7f403f8ae4af1a26178a8785a60d.js.map