  ...
  return {
    ...
    execute: function () {
      ...
      _export("Character", Character = (_dec = ccclass('Character'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Material), _dec5 = property(Material), _dec6 = property(Material), _dec7 = property(SkinnedMeshRenderer), _dec(_class = (_class2 = class Character extends (_crd && Entity === void 0 ? (_reportPossibleCrUseOfEntity({
        ...
        shakeRed() {
          let houseMaterial = this.node.getChildByName("playerNode").getChildByName("player").getChildByName("Shimin").getComponent(SkinnedMeshRenderer); // let materials = houseMaterial.materials;
        ...
        async collectWood(character) {
          ...
          let woodParent = character.node.getChildByName("backpack"); //woodParent.setScale(new Vec3(0.8,0.8,0.8));
        ...
        async collectWoodNew(character, num, target) {
          const prefab = await (_crd && ResourceManager === void 0 ? (_reportPossibleCrUseOfResourceManager({
            error: Error()
          }), ResourceManager) : ResourceManager).instance.loadPrefab((_crd && Global === void 0 ? (_reportPossibleCrUseOfGlobal({
            error: Error()
          }), Global) : Global).woodPrefabPath);
          let woodParent = character.node.getChildByName("backpack");
          ...
          for (let i = 0; i < num; i++) {
            ...
            this.scheduleOnce(() => {
              this.restoreItemsInBackpack1(pos, woodNode, character);
              (_crd && Global === void 0 ? (_reportPossibleCrUseOfGlobal({
                error: Error()
              }), Global) : Global).soundManager.playPickUpSound();
            }, i * 0.1);
        ...
        randomizeItemsInBackpack1(target, woodNode) {
          ...
          const parentWorldPos = target.node.getWorldPosition(); // 生成相对于父节点的随机偏移量
          ...
          let handOverPos = new Vec3(parentWorldPos.x + relativeOffset.x, parentWorldPos.y + relativeOffset.y, parentWorldPos.z + relativeOffset.z);
          const itemNode = woodNode;
          const itemWorldPos = itemNode.getWorldPosition().clone(); // 计算贝塞尔曲线控制点

          const LIFT_HEIGHT = 2;

          const randomLift = () => Math.floor(Math.random() * (LIFT_HEIGHT * 2 + 1)) - LIFT_HEIGHT;

          const controlPoint = new Vec3((itemNode.worldPosition.x + handOverPos.x) / 2 + randomLift(), (itemNode.worldPosition.y + handOverPos.y) / 2 + 6, (itemNode.worldPosition.z + handOverPos.z) / 2 + randomLift()); // 执行贝塞尔曲线动画
        ...
        restoreItemsInBackpack1(originalPos1, itemNode, character) {
          ...
          const currentPos = itemNode.getWorldPosition().clone(); // 贝塞尔曲线控制点

          const controlPoint = new Vec3((currentPos.x + originalPos.x) / 2, (currentPos.y + originalPos.y) / 2 + 5, (currentPos.z + originalPos.z) / 2); // for(let i = 0 ;i <5;i++){
          //     this.scheduleOnce(()=>{
          //         Global.soundManager.playPickUpSound()
          //     },i*0.1)
          // }
          ...
          tween(itemNode).to(0.3, {}, {
            ...
            onUpdate: (target, ratio) => {
              let woodParent = character.node.getChildByName("backpack");
              const newOriginalPos = new Vec3(woodParent.worldPosition.x, originalPos.y, woodParent.worldPosition.z);
              const position = (_crd && MathUtil === void 0 ? (_reportPossibleCrUseOfMathUtil({
                error: Error()
        ...
        async collectCorn(character) {
          const prefab = await (_crd && ResourceManager === void 0 ? (_reportPossibleCrUseOfResourceManager({
            error: Error()
          }), ResourceManager) : ResourceManager).instance.loadPrefab((_crd && Global === void 0 ? (_reportPossibleCrUseOfGlobal({
            error: Error()
          }), Global) : Global).cornPrefabPath);
          let woodParent1 = character.node.getChildByName("backpack1"); //let woodParent = (character as Character).node.getChildByName("backpack");
...
//# sourceMappingURL=d499ea35a94ff17d9e765a6bb57dfd7d99dbd090.js.map