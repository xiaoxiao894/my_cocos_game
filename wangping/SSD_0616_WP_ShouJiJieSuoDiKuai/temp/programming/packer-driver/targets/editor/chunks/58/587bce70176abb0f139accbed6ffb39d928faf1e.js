  ...
  return {
    ...
    execute: function () {
      ...
      _export("goodsDrop", goodsDrop = (_dec = ccclass('goodsDrop'), _dec(_class = class goodsDrop extends Component {
        ...
        randomizeItemsInBackpack(backpackIndex, count = 1) {
          let temp = -0.8;
          ...
          const parentWorldPos = backpack.parentNode.getWorldPosition(); // 生成相对于相对于父节点的随机偏移量
        ...
        restoreItemsInBackpack(backpackIndex) {
          ...
          const children = backpack.parentNode.children; // 计算需要归位的物品数量
          ...
          const itemsToRestore = children.length - ii; // 更新计数器
          ...
          for (let i = ii; i < children.length; i++) {
            ...
            (function (index) {
              ...
              this.scheduleOnce(() => {
                ...
                const controlPoint = new Vec3((currentPos.x + originalPos.x) / 2, (currentPos.y + originalPos.y) / 2 + 5, (currentPos.z + originalPos.z) / 2); // 播放音效
...
//# sourceMappingURL=587bce70176abb0f139accbed6ffb39d928faf1e.js.map