System.register(["cc"], function (_export, _context) {
  ...
  var _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, Vec3, v3, Quat, MathUtil, _crd, tempVec, tempVec2, tempVec3, up;
  ...
  return {
    ...
    execute: function () {
      ...
      _cclegacy._RF.push({}, "0e3f6eoRwlPIZup+z8Au50b", "MathUtils", undefined);
      ...
      _export("MathUtil", MathUtil = class MathUtil {
         ...
         */
        static generateSmoothPath(startPos, endPos, segmentCount = 100, curveHeight = 5) {
          // 计算结果数组
          const pathPoints = []; // 计算中间控制点(形成曲线顶部)
        ...
        static getAngleBetweenQuats(currentRot, targetRot) {
          // 计算两个四元数之间的点积
          const dot = Quat.dot(currentRot, targetRot); // 确保点积在有效范围内[-1, 1]

          const clampedDot = Math.min(1, Math.max(-1, dot)); // 返回角度差（弧度）
        ...
        static eulerToForward(eulerAngles) {
          ...
          const forward = new Vec3(0, 0, -1); // 3. 应用旋转
...
//# sourceMappingURL=6f02b7aba1b12e1bcce5192afc0d8cd0bec08dc4.js.map