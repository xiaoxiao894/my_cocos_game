...
export class UIJoyStickManager extends Component {
    ...
    private _showGuideDuration:number = 5;
    private _showGuideTimer:number = 5;
    private _showingGuide:boolean = false;

    private _isTouching = false;

    private _finalShowing:boolean = false;
    ...
    init() {
        ...
        DataManager.Instance.joyStick = this;
    ...
    protected onEnable(): void {
        ...
        EventManager.inst.on(EventName.ShowJoyStickFinal, this.showJoyStickFinal, this);
    }
    ...
    protected onDisable() : void {
        ...
        EventManager.inst.off(EventName.ShowJoyStickFinal, this.showJoyStickFinal, this);
    }

    onTouchStart(eventTouch: EventTouch) {
        if(this._finalShowing){
            this._finalShowing = false;
            this.opacityCom.opacity = 0;
            DataManager.Instance.sceneManger.showEndGameNode();
            return;
        }

        if (!DataManager.Instance.playerCanMove||DataManager.Instance.isGameOver) return;
    ...
    onTouchMove(touchEvent: EventTouch) {
        if (!DataManager.Instance.playerCanMove||DataManager.Instance.isGameOver){
            this.onTouchEnd(null);
             return;
        }
        if(this.opacityCom.opacity<=0){
    ...
    update(dt: number) {
        ...
        if ((!DataManager.Instance.playerCanMove||DataManager.Instance.isGameOver)&& !this._finalShowing) {
            this.onTouchEnd(null);
             return;
        }
    ...
    private showJoyStickFinal() {
        this._finalShowing = true;
        this._showGuideTimer = this._showGuideDuration;
    }
}
