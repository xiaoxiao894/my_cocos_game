    ...
    execute: function () {
      ...
      _export("WoodAccumulationConManager", WoodAccumulationConManager = (_dec = ccclass('WoodAccumulationConManager'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(Node), _dec6 = property(Node), _dec7 = property(Node), _dec8 = property(Animation), _dec(_class = (_class2 = class WoodAccumulationConManager extends Component {
        ...
        _processWood(wood) {
          ...
          const controlPoint = new Vec3((startPos.x + endPos.x) / 2, Math.max(startPos.y, endPos.y) + 15, (startPos.z + endPos.z) / 2);
          const level = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
        ...
        _updateBezierMovement(wood, startPos, controlPoint, endPos, startRot, endRot, t) {
          const oneMinusT = 1 - t; // 三次贝塞尔插值位置

          const pos = new Vec3(oneMinusT * oneMinusT * startPos.x + 2 * oneMinusT * t * controlPoint.x + t * t * endPos.x, oneMinusT * oneMinusT * startPos.y + 2 * oneMinusT * t * controlPoint.y + t * t * endPos.y, oneMinusT * oneMinusT * startPos.z + 2 * oneMinusT * t * controlPoint.z + t * t * endPos.z);
          wood.setWorldPosition(pos); // 欧拉角插值

          const euler = new Vec3(startRot.x * oneMinusT + endRot.x * t, startRot.y * oneMinusT + endRot.y * t, startRot.z * oneMinusT + endRot.z * t);
        ...
        _onBezierComplete(wood, endRot) {
          ...
          const moveEnd = this.cutterEnd.worldPosition.clone(); // const level: number = DataManager.Instance.conveyorLevel;
          ...
          tween(moveParam).to(0.2, {
            ...
            onUpdate: () => {
              const t = moveParam.t;
              const pos = new Vec3(moveStart.x + (moveEnd.x - moveStart.x) * t, moveStart.y + (moveEnd.y - moveStart.y) * t, moveStart.z + (moveEnd.z - moveStart.z) * t);
        ...
        getTopItem() {
          ...
          const node = (lastSlot == null ? void 0 : lastSlot.assignedNode) || null;
