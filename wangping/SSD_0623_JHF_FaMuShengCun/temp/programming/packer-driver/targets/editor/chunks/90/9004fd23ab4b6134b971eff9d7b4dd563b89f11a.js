    ...
    execute: function () {
      ...
      _export("ItemPartnerManager", ItemPartnerManager = (_dec = ccclass('ItemPartnerManager'), _dec2 = property(SkeletalAnimation), _dec(_class = (_class2 = class ItemPartnerManager extends Component {
        ...
        handleTreeApproachAndCut(deltaTime) {
          ...
          const targetPos = this._target.worldPosition; // 记录初始移动位置（只记录一次）
        ...
        playAttackAnimation(target) {
          ...
          let tree = target.getComponent(_crd && ItemTreeManager === void 0 ? (_reportPossibleCrUseOfItemTreeManager({
            error: Error()
          ...
          this.scheduleOnce(() => {
            if (target != null && target.isValid && target.parent) {
              const tree = target.getComponent(_crd && ItemTreeManager === void 0 ? (_reportPossibleCrUseOfItemTreeManager({
                error: Error()
              }), ItemTreeManager) : ItemTreeManager);
        ...
        updateTargetIfNeeded() {
          const trees = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
        ...
        startSequentialDelivery() {
          ...
          if (!to) return;
          const stackManager = to["__stackManager"]; // 从最后一个有效的准备 item 开始
