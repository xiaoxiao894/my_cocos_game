  ...
  return {
    ...
    execute: function () {
      ...
      _export("DissolveEffect", DissolveEffect = (_dec = ccclass('DissolveEffect'), _dec2 = property([Material]), _dec(_class = (_class2 = class DissolveEffect extends Component {
        ...
        start() {//this._createMaterialInstances();
        } //
        ...
        _createMaterialInstances() {
          ...
          const originalMaterials = renderer.materials; // 创建材质实例

          for (let i = 0; i < this.mats.length; i++) {
            const originalMat = this.mats[i];

            if (originalMat) {
              let matInstance = null; // 尝试使用 setMaterial() 方法创建实例（通用方法）
              // 临时应用材质以创建实例
          ...
          if (this._matInstances.length > 0) {
            renderer.materials = this._matInstances; //console.log(`DissolveEffect: 成功为 ${this.node.name} 创建了 ${this._matInstances.length} 个材质实例`);
          } else {// console.error("DissolveEffect: 未能创建任何材质实例");
          }
        }

        onEnable() {//this._resetTimes = 0;
        }
...
//# sourceMappingURL=fce78db0791fb6c46cdbd833e332ac615cdac002.js.map