  ...
  return {
    ...
    execute: function () {
      ...
      _export("SceneCoinConManager", SceneCoinConManager = (_dec = ccclass('SceneCoinConManager'), _dec(_class = class SceneCoinConManager extends Component {
        ...
        update(deltaTime) {
          const iconList = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          ...
          for (let i = 0; i < iconList.length; i++) {
            ...
            if (!slot) continue; // 当前占用的槽位与索引（飞行途中可能被“降档”到更前的空槽）
            ...
            tween(tParam).to(1, {
              ...
              onUpdate: () => {
                const t = tParam.t;
                const oneMinusT = 1 - t; // 尝试“降档补洞”：若前方出现更靠前的空槽，更新占用关系
                ...
                const endPosNow = stackManager.getSlotWorldPos(curSlot, this.node); // 每帧重算控制点

                const ctrlX = (startPos.x + endPosNow.x) * 0.5;
                const ctrlY = (startPos.y + endPosNow.y) * 0.5 + 20;
                const ctrlZ = (startPos.z + endPosNow.z) * 0.5; // 二次贝塞尔插值

                const pos = new Vec3(oneMinusT * oneMinusT * startPos.x + 2 * oneMinusT * t * ctrlX + t * t * endPosNow.x, oneMinusT * oneMinusT * startPos.y + 2 * oneMinusT * t * ctrlY + t * t * endPosNow.y, oneMinusT * oneMinusT * startPos.z + 2 * oneMinusT * t * ctrlZ + t * t * endPosNow.z);
