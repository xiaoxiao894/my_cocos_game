    ...
    execute: function () {
      ...
      _export("PlayerManager", PlayerManager = (_dec = ccclass('PlayerManager'), _dec2 = requireComponent(_crd && Actor === void 0 ? (_reportPossibleCrUseOfActor({
        ...
        update(deltaTime) {
          ...
          var jackParent = (_this$node = this.node) == null ? void 0 : _this$node.getChildByName("JackParent");
          var playerJack = jackParent == null ? void 0 : jackParent.getChildByName("Player_Jack");
          var shanxing = playerJack.getChildByName("shanxing");
          ...
          var hasMonsters = this.trees && this.trees.length > 0;
          ...
          if (!this._isAttacking) {
            if (len > 0.1) {
              ...
              if (hasMonsters) {
                ...
                // === 空闲状态动画处理（骨骼动画冻结一帧） ===
                var _jackParent = this.node.getChildByName("JackParent");

                var jackParentAni = _jackParent == null ? void 0 : _jackParent.getComponent(Animation);
                ...
                var jack = _jackParent.getChildByName("Player_Jack");
        ...
        _onAttackFinished(anim, state) {
          if (state.name === "TX_Attack") {
            ...
            var jack = jackParent.getChildByName("Player_Jack");
        ...
        getWoods(dt) {
          ...
          var woodList = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          ...
          var playerPos = player.worldPosition.clone();
          var maxDistanceXZ = 33;
          var allwoods = [...woodList, ...this._uncollectedWood];
          this._uncollectedWood = [];
          var delayCounter = 0;
          ...
          var _loop = function _loop() {
            ...
            var woodPos = wood.worldPosition.clone();
            var dx = woodPos.x - playerPos.x;
            var dz = woodPos.z - playerPos.z;
            var distSqrXZ = dx * dx + dz * dz;

            if (distSqrXZ > maxDistanceXZ * maxDistanceXZ) {
              _this._uncollectedWood.push(wood);

              return 0; // continue
            }

            var targetBackpack = findTargetBackpack(wood);
            ...
            wood.setParent(_this.node.parent);
            ...
            var startRot = wood.eulerAngles.clone(); // 初始角度

            var endRot = new Vec3(0, 0, 90); // 最终角度
            ...
            tween(controller).delay(delayCounter * 0.05).to(duration, {
              ...
              onUpdate: () => {
                var t = controller.t;
                var oneMinusT = 1 - t; // 计算当前目标背包的堆叠高度

                var maxY = 0;

                for (var child of targetBackpack.children) {
                ...
                var localTarget = new Vec3(0, maxY + 0.5, 0); // 背包的世界变换矩阵
                ...
                var worldTarget = Vec3.transformMat4(new Vec3(), localTarget, worldMat); // 控制点

                var control = new Vec3((start.x + worldTarget.x) / 2, Math.max(start.y, worldTarget.y) + 2, (start.z + worldTarget.z) / 2); // 贝塞尔插值
        ...
        // 解锁Plot8 
        unlockPlot8() {
          var plot8 = find("THREE3DNODE/Unlock/Plot8");
          plot8.active = true;
