    ...
    execute: function () {
      ...
      _export("MonsterConManager", MonsterConManager = (_dec = ccclass('MonsterConManager'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec(_class = (_class2 = class MonsterConManager extends Component {
        ...
        init() {
          ...
          for (var i = 0; i < this._monsterTypes.length; i++) {
          ...
          var _loop = function _loop() {
            var monster = _this.node.children[_i];
            ...
            var validPos = monster.worldPosition;

            var idx = _this._monsterPools.findIndex(item => {
            ...
            var agentId = (_crd && Simulator === void 0 ? (_reportPossibleCrUseOfSimulator({
              ...
              error: Error()
            }), Util) : Util).v3t2(validPos), _this._radiusCfg[idx], _this._speedCfg[idx], null, mass);
            var agentObj = (_crd && Simulator === void 0 ? (_reportPossibleCrUseOfSimulator({
              error: Error()
            }), Simulator) : Simulator).instance.getAgentByAid(agentId);
            agentObj.neighborDist = _this._radiusCfg[idx] * 2;
        ...
        creatMonster(isDissolveOnce, isAfferentPos, pos, isCustomHp) {
          if (isAfferentPos === void 0) {
            isAfferentPos = false;
          }
          ...
          if (isCustomHp === void 0) {
            isCustomHp = false;
          }

          var randomNum = this.getWeightedRandom();
          var pool = this._monsterPools[randomNum];
          ...
          var node = pool.getItem();
          var monster = node.getComponent(_crd && MonsterItem === void 0 ? (_reportPossibleCrUseOfMonsterItem({
            error: Error()
          }), MonsterItem) : MonsterItem);
          if (!monster) return;
          var validPos = null;
          ...
          if (isAfferentPos) {
            ...
            var birthPoints = this.monsterBirthPoint.children;

            if (this.selectLocationIndex >= birthPoints.length) {
              this.selectLocationIndex = 0;
            }

            var birthPoint = birthPoints[this.selectLocationIndex];
        ...
        getWeightedRandom() {
          var weights = [25, 25, 25, 25]; // 对应 0, 1, 2 的权重
        ...
        update(dt) {
          if ((_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            ...
            if (this._monsterPools.length > 0) {
              var currentMonsterCount = this.monsterParent.children.length;
              ...
              if (currentMonsterCount < (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
                ...
                if (this._bornTime > (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
                  ...
                  var spawnCount = Math.min(this._bornSpeed, (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
                    error: Error()
            ...
            for (var index = 0; index < this.monsterParent.children.length; index++) {
              var _monster$getComponent;

              var monster = this.monsterParent.children[index];
        ...
        dropItem(pos, end) {
          var node = this._dropPool.getItem();
          ...
          var startY = pos.y + 0.5;
          var peakY = startY + 3; // 第一次跃起高度

          var bounceY = startY + 0.7; // 回落后的弹跳高度

          var targetPos = new Vec3(pos.x, pos.y, pos.z - 3);
          var startRot = node.rotation.clone();
          var p0 = end; //.worldPosition //.clone();// node.worldPosition.clone(); // 起点（此时在地面）

          var p2 = targetPos.clone(); // 终点
          // 控制点：取中点并抬高一些，形成明显弧线；你也可以把 2.5 调高/调低

          var p1 = new Vec3((p0.x + p2.x) * 0.5, Math.max(p0.y, p2.y) + 15, (p0.z + p2.z) * 0.5);
          ...
          tween(param).to(0.5, {
            ...
            onUpdate: () => {
              var t = param.t;
              var u = 1 - t; // 二次贝塞尔：B(t) = u^2 * p0 + 2*u*t * p1 + t^2 * p2

              var x = u * u * p0.x + 2 * u * t * p1.x + t * t * p2.x;
              var y = u * u * p0.y + 2 * u * t * p1.y + t * t * p2.y;
              var z = u * u * p0.z + 2 * u * t * p1.z + t * t * p2.z;
        ...
        getDrops() {
          var newList = this._coinsList.splice(0, Math.min(this._coinsList.length, 1));

          return newList;
        }

        startSelfRotate(node, speedDegPerSec, axis) {
          if (speedDegPerSec === void 0) {
            speedDegPerSec = 60;
          }
          ...
          var dur = 10; // tween 动画时长

          var delta = speedDegPerSec * dur; // 总旋转角度

          var by = axis === 'x' ? new Vec3(delta, 0, 0) : axis === 'y' ? new Vec3(0, delta, 0) : new Vec3(0, 0, delta);
          var tw = tween(node).repeatForever(tween(node).to(dur, {
            eulerAngles: by
          })).start();
          node[SPIN_KEY] = tw;
        }
        ...
        stopSelfRotate(node) {
          var tw = node == null ? void 0 : node[SPIN_KEY];
