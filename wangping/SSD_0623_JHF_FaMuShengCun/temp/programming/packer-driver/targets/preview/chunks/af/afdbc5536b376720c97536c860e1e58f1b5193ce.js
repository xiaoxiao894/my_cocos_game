  ...
  return {
    ...
    execute: function () {
      ...
      _export("SearchTreeManager", SearchTreeManager = (_dec = ccclass('SearchTreeManager'), _dec(_class = class SearchTreeManager extends Component {
        ...
        getAttackTargets(player, attackRange, maxAngle, isPlayer) {
          if (isPlayer === void 0) {
            isPlayer = false;
          }
          ...
          var nearby = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          }), DataManager) : DataManager).Instance.gridSystem.getNearbyNodes(player.worldPosition, attackRange);
          var forward = player.forward.clone().normalize();
          var result = [];

          for (var i = 0; i < nearby.length; i++) {
            var enemy = nearby[i];
            if (!enemy.activeInHierarchy) continue;
            var toEnemy = enemy.worldPosition.clone().subtract(player.worldPosition);
            var dist = toEnemy.length();
            if (dist > attackRange) continue;
            toEnemy.normalize();
            var angle = Math.acos(Vec3.dot(forward, toEnemy)) * 180 / Math.PI;
