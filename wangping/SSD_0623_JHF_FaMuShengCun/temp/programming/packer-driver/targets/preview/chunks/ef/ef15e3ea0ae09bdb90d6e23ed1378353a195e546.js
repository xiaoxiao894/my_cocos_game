  ...
  return {
    ...
    execute: function () {
      ...
      _export("MonsterItem", MonsterItem = (_dec = ccclass('MonsterItem'), _dec2 = property(SkeletalAnimation), _dec3 = property(Node), _dec4 = property(CCString), _dec5 = property(CCString), _dec6 = property(CCInteger), _dec7 = property(_crd && DissolveEffect === void 0 ? (_reportPossibleCrUseOfDissolveEffect({
        ...
        init(index, isDissolveOnce, isCustomHp) {
          if (index === void 0) {
            index = 0;
          }

          if (isDissolveOnce === void 0) {
            isDissolveOnce = true;
          }
          ...
          if (this.bloodNode) {
            var bar = this.bloodNode.getComponent(ProgressBar);
        ...
        deathAni(end) {
          ...
          var knockbackDir = new Vec3(0, 0, -1); // 固定击退距离

          var knockbackDistance = 0.75; // 击退 3 个单位

          var goalVector = knockbackDir.multiplyScalar(knockbackDistance);
          tween(this.node).by(0.15, {
            ...
            if (agent && (_this$node = this.node) != null && _this$node.isValid) {
              var newWorldPos = this.node.worldPosition;
        ...
        updateIconPos(end) {
          ...
          var worldPos = this.node.getWorldPosition().clone();

          for (var i = 0; i < this.dropNum; i++) {
            var randius = 3;
            var angle = Math.random() * Math.PI * 2;
            var r = Math.sqrt(Math.random()) * randius;
            var offsetX = r * Math.cos(angle);
            var offsetZ = r * Math.sin(angle);
            var newRandomPos = new Vec3(worldPos.x + offsetX, worldPos.y, worldPos.z + offsetZ);
            (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              error: Error()
        ...
        setPreferredVelocities(dt) {
          ...
          var moveTarget;
          var worldTarget;
          var monsterPos = this.node.worldPosition.clone();
          var nearestGuardrail = null;
          var minDistSqr = Infinity;

          for (var guardrail of (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          }), DataManager) : DataManager).Instance.guardrailArr) {
            var attackingCount = guardrail.attackingMonsterCount || 0;
            if (attackingCount >= 2) continue;
            var guardrailPos = guardrail.node.worldPosition;
            var dx = guardrailPos.x - monsterPos.x;
            var dz = guardrailPos.z - monsterPos.z;
          ...
          if (nearestGuardrail && !this._assignedGuardrail) {
            var pos = nearestGuardrail.node.worldPosition;
            var dir = new Vec3();
            Vec3.subtract(dir, pos, monsterPos);
            dir.y = 0;
            dir.normalize();
            var offset = 4;
            var adjustedPos = pos.clone().subtract(dir.multiplyScalar(offset));
            ...
            var _guardrailPos = this._assignedGuardrail.node.worldPosition;

            var _dir = new Vec3();

            Vec3.subtract(_dir, _guardrailPos, monsterPos);
            _dir.y = 0;
            ...
            var _adjustedPos = _guardrailPos.clone().subtract(_dir.multiplyScalar(_offset));

            moveTarget = v2(_adjustedPos.x, _adjustedPos.z);
            worldTarget = new Vec3(_adjustedPos.x, 0, _adjustedPos.z);
          } else {
            for (var _guardrail of (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              error: Error()
            }), DataManager) : DataManager).Instance.guardrailArr) {
              var _guardrailPos2 = _guardrail.node.worldPosition;

              var _dx = _guardrailPos2.x - monsterPos.x;

              var _dz = _guardrailPos2.z - monsterPos.z;

              var _distSqr = _dx * _dx + _dz * _dz;

              if (_distSqr < minDistSqr) {
                minDistSqr = _distSqr;
                nearestGuardrail = _guardrail;
              }
            }

            if (nearestGuardrail) {
              var _pos = nearestGuardrail.node.worldPosition;

              var _dir2 = new Vec3();

              Vec3.subtract(_dir2, _pos, monsterPos);
              _dir2.y = 0;
              ...
              var _adjustedPos2 = _pos.clone().subtract(_dir2.multiplyScalar(_offset2));

              moveTarget = v2(_adjustedPos2.x, _adjustedPos2.z);
              worldTarget = new Vec3(_adjustedPos2.x, 0, _adjustedPos2.z);
            }
          }
          ...
          if (agent && agentPos) {
            var goalVector = moveTarget.subtract2f(agentPos.x, agentPos.y);

            var _distSqr2 = goalVector.lengthSqr();

            var distanceToTargetSquared = Vec3.squaredDistance(this.node.worldPosition, worldTarget);
            var hasArrived = distanceToTargetSquared <= this.sizeSquare;
            ...
            if (_distSqr2 < (_crd && RVOMath === void 0 ? (_reportPossibleCrUseOfRVOMath({
              error: Error()
            ...
            var forward = new Vec3(goalVector.x, 0, goalVector.y).normalize();

            if (forward.lengthSqr() > 0.0001) {
              var currentRotation = this.skeletalAnimation.node.worldRotation.clone();
              var targetRotation = new Quat();
              Quat.fromViewUp(targetRotation, forward, Vec3.UP);
              var rotateSpeed = 8;
              var slerped = new Quat();
              Quat.slerp(slerped, currentRotation, targetRotation, Math.min(1, dt * rotateSpeed));
              var childZero = this.skeletalAnimation.node;
        ...
        rotateTowards(targetWorldPos, dt) {
          ...
          var currentQuat = this.skeletalAnimation.node.worldRotation.clone();
        ...
        changState(state) {
          ...
          if (state === (_crd && MonsterStateEnum === void 0 ? (_reportPossibleCrUseOfMonsterStateEnum({
            ...
            var _state = this.skeletalAnimation.getState((_crd && MonsterStateEnum === void 0 ? (_reportPossibleCrUseOfMonsterStateEnum({
              error: Error()
            }), MonsterStateEnum) : MonsterStateEnum).Attack);

            if (!_state) return;

            _state.update(0); // 强制立即应用该时间的骨骼姿势
        ...
        attack() {
          ...
          if (this._assignedGuardrail) {
            ...
            if (this._assignedGuardrail.blood > 0) {
              ...
              var bloodNode = this._assignedGuardrail.node.getChildByName("FenceBloodBar");
              ...
              var bloodBar = bloodNode.getComponent(ProgressBar);
            ...
            if (this._assignedGuardrail.node.name.includes("Door")) {
              //门
              var leftNode = this._assignedGuardrail.node.getChildByPath("Door_Left/WeiLan/WeiLan");

              if (leftNode) {
                var leftMesh = leftNode.getComponent(MeshRenderer);
              ...
              var rightNode = this._assignedGuardrail.node.getChildByPath("Door_Right/WeiLan/WeiLan");

              if (rightNode) {
                var rightMesh = rightNode.getComponent(MeshRenderer);
              ...
              //围栏
              var fenceNode = this._assignedGuardrail.node.getChildByName("weiqiang02");

              if (fenceNode) {
                var fenceMesh = fenceNode.getComponent(MeshRenderer);
