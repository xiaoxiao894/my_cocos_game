      ...
      _export("PlayerTurret", PlayerTurret = (_dec = ccclass('PlayerTurret'), _dec2 = property({
        ...
        AttackAni() {
          ...
          if (!this.isInAttackRange(currentAttackTarget.node)) {
            ...
            this.enemyHitPos = currentAttackTarget.hitPosNode.worldPosition.clone(); // return;
          }
          ...
          const flightTime = Math.max(0.1, distance / this.bulletSpeed); // // 子弹飞行动画
          // tween(prefab)
          ...
          const controlPoint = new Vec3((startPos.x + endPos.x) / 2, Math.max(startPos.y, endPos.y) + LIFT_HEIGHT, // 取较高点上方
        ...
        explodeAtPosition(position) {
          ...
          // 获取所有敌人列表
          const allEnemies = [...(_crd && App === void 0 ? (_reportPossibleCrUseOfApp({
            error: Error()
          ...
          const affectedEnemies = allEnemies.filter(enemy => this.isValidEnemy(enemy) && this.getDistanceSquared(enemy.node.worldPosition, position) <= this.explosionRangeSquared); // 对所有受影响的敌人造成伤害
        ...
        rotateTowards(targetWorldPos, dt) {
          ...
          const resultQuat = new Quat(); // 平滑插值旋转
