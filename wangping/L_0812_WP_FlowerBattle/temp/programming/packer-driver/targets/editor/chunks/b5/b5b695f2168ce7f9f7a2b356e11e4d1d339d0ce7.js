  ...
  return {
    ...
    execute: function () {
      ...
      _export("EnemySpider", EnemySpider = (_dec = ccclass('EnemySpider'), _dec2 = property(Material), _dec3 = property(Material), _dec4 = property(SkinnedMeshRenderer), _dec5 = property(_crd && DissolveEffect === void 0 ? (_reportPossibleCrUseOfDissolveEffect({
        ...
        init() {
          let data = (_crd && App === void 0 ? (_reportPossibleCrUseOfApp({
            error: Error()
        ...
        baseHit(attack, bulletPos, hitRange = 1) {
          if (this.hp <= 0) return;
        ...
        hitEffct(back = null) {
          ...
          let arrowTx = (_crd && App === void 0 ? (_reportPossibleCrUseOfApp({
            error: Error()
        ...
        baseHit1(attack, bulletPos, hitRange = 1) {
          if (this.hp <= 0) return;
          if (!this.isAttack) return; // this.hitParticle.node.parent.active = true;
          // this.hitParticle.play();
        ...
        turretHit(attack, bulletPos, hitRange = 8) {
          if (this.hp <= 0) return;
        ...
        attackEvent() {
          if ((_crd && GlobeVariable === void 0 ? (_reportPossibleCrUseOfGlobeVariable({
            ...
            let blood = (_crd && App === void 0 ? (_reportPossibleCrUseOfApp({
              error: Error()
        ...
        moveByRvo(dt) {
          ...
          let block = (_crd && App === void 0 ? (_reportPossibleCrUseOfApp({
            error: Error()
          }), App) : App).sceneNode.moveBlockPos.children[this.blockIndex];
          block.getWorldPosition(playerPos); // 计算距离

          const distance = Vec3.distance(currentPos1, playerPos); // 如果距离小于等于攻击距离，则执行攻击逻辑
        ...
        */
        ...
        setPreferredVelocities(hitPow = null) {
          if (this.agentHandleId < 0) {
            return;
          }

          let agentAid = this.agentHandleId;
          let agent = (_crd && Simulator === void 0 ? (_reportPossibleCrUseOfSimulator({
            error: Error()
          }), Simulator) : Simulator).instance.getAgentByAid(agentAid);
          let agentPos = (_crd && Simulator === void 0 ? (_reportPossibleCrUseOfSimulator({
            error: Error()
          ...
          let moveTargetPos = v2(playerPos.x, playerPos.z);

          if (hitPow) {
            if (agent && agentPos) {
              let goalVector = moveTargetPos.subtract2f(agentPos.x, agentPos.y);
          ...
          if (agent && agentPos) {
            ...
            if (goalVector.lengthSqr() < (_crd && RVOMath === void 0 ? (_reportPossibleCrUseOfRVOMath({
              error: Error()
        ...
        die(callback) {
          ...
          const playerPos = this.hitRangeV3; // 计算击退方向向量
        ...
        randomPos() {
          // 获取原位置并克隆
          const originalPos = this.node.getWorldPosition().clone(); // 生成随机方向向量并归一化

          const randomDir = new Vec3(Math.random() * 2 - 1, // x轴: -1 到 1
          0, // y轴: -1 到 1
          ...
          let POS = (_crd && App === void 0 ? (_reportPossibleCrUseOfApp({
            error: Error()
        ...
        update(deltaTime) {
          if ((_crd && GlobeVariable === void 0 ? (_reportPossibleCrUseOfGlobeVariable({
            ...
            let selfPos = this.node.worldPosition; // // 计算两点到世界原点的距离
            ...
            const forward = this.node.forward; // 计算目标相对于自身的向量

            const toTarget = pos.clone().subtract(selfPos); // 点积判断是否在前方（大于0为前方）
