    ...
    execute: function () {
      ...
      _export("Player", Player = (_dec = ccclass('Player'), _dec2 = property(Animation), _dec(_class = (_class2 = class Player extends (_crd && Entity === void 0 ? (_reportPossibleCrUseOfEntity({
        ...
        continue() {
          ...
          for (let i = 0; i < (_crd && GlobeVariable === void 0 ? (_reportPossibleCrUseOfGlobeVariable({
            error: Error()
          }), GlobeVariable) : GlobeVariable).coinStartNum; i++) {
            let node = (_crd && App === void 0 ? (_reportPossibleCrUseOfApp({
              error: Error()
        ...
        collectCoin() {
          ...
          if ((_crd && App === void 0 ? (_reportPossibleCrUseOfApp({
            ...
            let coin = (_crd && App === void 0 ? (_reportPossibleCrUseOfApp({
              error: Error()
        ...
        coinFly(coin) {
          ...
          const startRot = coin.worldRotation.clone();
          const endRot = this.backpack.worldRotation; // 保存tween引用以便后续取消
          ...
          this.coinTween = tween(controller).to(duration, {
            ...
            onUpdate: () => {
              ...
              const pos = new Vec3(oneMinusT * oneMinusT * start.x + 2 * oneMinusT * t * control.x + t * t * worldTarget.x, oneMinusT * oneMinusT * start.y + 2 * oneMinusT * t * control.y + t * t * worldTarget.y, oneMinusT * oneMinusT * start.z + 2 * oneMinusT * t * control.z + t * t * worldTarget.z); // 只在coin有效时设置位置

              coin.setWorldPosition(pos);
              const lerpedEuler = new Quat(startRot.x * oneMinusT + endRot.x * t, startRot.y * oneMinusT + endRot.y * t, startRot.z * oneMinusT + endRot.z * t, startRot.w * oneMinusT + endRot.w * t); // 只在coin有效时设置旋转
