      ...
      _export("PlayerPylon", PlayerPylon = (_dec = ccclass('PlayerPylon'), _dec2 = property({
        ...
        AttackAni() {
          ...
          if (this.currentTarget) {
            ...
            var flightTime = Math.max(0.05, distance / this.bulletSpeed); // 保存初始目标位置，防止目标移动导致子弹跟踪到出生点

            var initialTargetPos = this.enemyHitPos.clone();
            tween(prefab).to(flightTime, {
              ...
              onUpdate: (target, ratio) => {
                ...
                var currentPos = target.worldPosition;
                var dir = new Vec3();
                Vec3.subtract(dir, initialTargetPos, currentPos);
                dir.normalize();

                if (dir.lengthSqr() > 0.0001) {
                  var oppositeDir = new Vec3(-dir.x, -dir.y, -dir.z);
        ...
        rotateTowards(targetWorldPos, dt) {
          ...
          var oppositeDir = new Vec3(-dir.x, -dir.y + 0.1, -dir.z - 0.1);
          var targetQuat = new Quat();
          Quat.fromViewUp(targetQuat, oppositeDir, Vec3.UP);
