  ...
  return {
    ...
    execute: function () {
      ...
      _export("PlayerAttackFlower", PlayerAttackFlower = (_dec = ccclass('PlayerAttackFlower'), _dec2 = property({
        ...
        digestProgress() {
          var digestProgress = 1 - this.testInterval / this.attackInterval;

          if (this.iseatSpiderMove) {
            var z = this.spaider.position.z;
            var x = this.spaider.position.x;
            this.spaider.setPosition(x -= 0.003, this.spaider.position.y, z += 0.0015);
          }

          if (digestProgress <= 0) {
            // 获取当前缩放
            var currentScale = this.spaider.scale; // 计算新缩放值（向目标缩放靠近）

            var newScaleX = currentScale.x - 0.002;
            var newScaleY = currentScale.y - 0.002;
            var newScaleZ = currentScale.z - 0.002; // 设置新缩放
        ...
        onTriggerEnter(event) {
          ...
          var selfnode = event.otherCollider.node; //selfnode.getComponent(EnemySpider).node.active = false;
        ...
        attackList() {
          // 可攻击的怪物
          // 获取当前节点的世界位置
          var selfPos = this.node.worldPosition; // 存储在120度范围内的敌人

          var enemiesInSector = []; // 遍历攻击目标列表

          for (var enemy of this.attackTargetList) {
            ...
            var angle = Math.atan2(direction.x, direction.z) * 180 / Math.PI; // 取绝对值，判断是否在120度范围内（-60度到+60度之间）
        ...
        update(deltaTime) {
          if (!this.isEat) {
            ...
            if (this.testInterval > this.attackInterval) {
              this.testInterval = 0;
              this.checkEnemy();
              var attackList = this.attackList();
