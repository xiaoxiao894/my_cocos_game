  ...
  return {
    ...
    execute: function () {
      ...
      _export("BeetleController", BeetleController = class BeetleController {
        ...
        removeBeetle(beetle) {
          var index = this.beetleList.indexOf(beetle);
        ...
        realCreatBeetle(pos) {
          ...
          prefab.active = true;
          var beetle = prefab.getComponent(_crd && PlayerBeetle === void 0 ? (_reportPossibleCrUseOfPlayerBeetle({
            error: Error()
        ...
        update(dt) {
          for (var index = 0; index < this.beetleList.length; index++) {
            var _enemy$getComponent;

            var enemy = this.beetleList[index];
