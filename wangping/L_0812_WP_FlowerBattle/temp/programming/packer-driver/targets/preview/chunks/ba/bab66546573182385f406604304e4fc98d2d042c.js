    ...
    execute: function () {
      ...
      _export("MapShowController", MapShowController = (_dec = ccclass('MapShowController'), _dec2 = property({
        ...
        continueGame() {
          ...
          var arrowTowerModel = this.arrowTower.getChildByName("HuoChaiHeJiaTa").getChildByName("model").getChildByName("L_prp_HuoChaiHe_jt_Skin_V001");
        ...
        continueShowLable() {
          (_crd && GlobeVariable === void 0 ? (_reportPossibleCrUseOfGlobeVariable({
            ...
            var passData = (_crd && GlobeVariable === void 0 ? (_reportPossibleCrUseOfGlobeVariable({
              error: Error()
            }), GlobeVariable) : GlobeVariable).handVoer[element];
            var nodes = this.buidingAreaParent.getChildByName(element);

            if (nodes) {
              var _nodes$getChildByName;

              var lab = (_nodes$getChildByName = nodes.getChildByName("UnlockTileLabel")) == null ? void 0 : _nodes$getChildByName.getComponent(Label);
              lab.string = passData.maxCoin + "";
              var sp = nodes.getChildByName("progress").getComponent(Sprite);
              sp.fillRange = 0;
        ...
        bubbleAniSpeed(speed) {
          if (speed === void 0) {
            speed = 0.7;
          }

          var ani = this.bubble.getComponent(Animation);
          var state = ani.getState("qipaohuxi");
        ...
        showLable() {
          (_crd && GlobeVariable === void 0 ? (_reportPossibleCrUseOfGlobeVariable({
            ...
            if (nodes) {
              ...
              var lab = (_nodes$getChildByName2 = nodes.getChildByName("UnlockTileLabel")) == null ? void 0 : _nodes$getChildByName2.getComponent(Label);
              lab.string = passData.maxCoin + "";
            } // passData.isShow = true;
            // passData.showCoin = passData.maxCoin;
        ...
        setBubbleLable() {
          var remaining = (_crd && GlobeVariable === void 0 ? (_reportPossibleCrUseOfGlobeVariable({
            error: Error()
          ...
          var sp = this.bubble.getChildByName("qipao").getChildByName("qipao-001").getComponent(Sprite);
        ...
        restBlock1() {
          ...
          var sp = this.block1Area.getChildByName("progress").getComponent(Sprite);
          var lb = this.block1Area.getChildByName("UnlockTileLabel").getComponent(Label); // 缓存当前关卡数据
        ...
        showUIBuiding(areaName) {
          if (areaName == "UI_jianta") {
            //解锁箭塔后显示拒马交付区域
            var jiantou = find("Physice/UI_jianta");
            var collider = jiantou.getComponent(BoxCollider);

            if (collider) {
              collider.enabled = true;
            }

            var rigidBody = jiantou.getComponent(RigidBody);
          ...
          if (areaName == "UI_paota") {
            var _jiantou = find("Physice/UI_paota");

            var _collider = _jiantou.getComponent(BoxCollider);

            if (_collider) {
              _collider.enabled = true;
            }

            var _rigidBody = _jiantou.getComponent(RigidBody);
          ...
          if (areaName == "UI_jiachong") {
            var _jiantou2 = find("Physice/UI_jiachong");

            var _collider2 = _jiantou2.getComponent(BoxCollider);

            if (_collider2) {
              _collider2.enabled = true;
            }

            var _rigidBody2 = _jiantou2.getComponent(RigidBody);

            if (_rigidBody2) {
              _rigidBody2.enabled = true;
            }
