    ...
    execute: function () {
      ...
      _export("PlayerTrigger", PlayerTrigger = (_dec = ccclass('Trigger'), _dec(_class = class PlayerTrigger extends Component {
        ...
        initTrigger(player) {
          //this.triggerNode = node;
          this.player = player;
          var collider = this.player.node.getComponent(Collider);
        ...
        onTriggerEnter(event) {
          ...
          var nodeName = event.otherCollider.node.name;
        ...
        //事件监听触发
        onTriggerStay(event) {
          var nodeName = event.otherCollider.node.name; //         if (GlobeVariable.handOverArea.indexOf(nodeName) != -1) {
          //             console.log("交付区域")
          //             const tnode = event.otherCollider.node;
          //             let { curCoin: cur, maxCoin: max } = GlobeVariable.handVoer[event.otherCollider.node.name];
          //             // 检查交付条件
          ...
          if ((_crd && GlobeVariable === void 0 ? (_reportPossibleCrUseOfGlobeVariable({
            ...
            if ((_this$player$backpack = this.player.backpack) != null && _this$player$backpack.children.length && cur < max) {
              ...
              this.time = 0;
              var lastItem = this.player.backpack.children[this.player.backpack.children.length - 1];
              var pos = lastItem.worldPosition.clone();
              this.player.subtractCoin(1);
              (_crd && GlobeVariable === void 0 ? (_reportPossibleCrUseOfGlobeVariable({
                error: Error()
              ...
              var handOverLevelPos = (_crd && App === void 0 ? (_reportPossibleCrUseOfApp({
                error: Error()
        ...
        showProgressBubbleLevel(node) {
          ...
          passData.showCoin++;
          var remaining = passData.maxCoin - passData.showCoin; //passData.curCoin;
          // 处理边界值
        ...
        showProgressLevel(node) {
          // this.curCoin++;
          // 简化组件获取
          var sp = node.getChildByName("progress").getComponent(Sprite);
          var lb = node.getChildByName("UnlockTileLabel").getComponent(Label); // 缓存当前关卡数据
