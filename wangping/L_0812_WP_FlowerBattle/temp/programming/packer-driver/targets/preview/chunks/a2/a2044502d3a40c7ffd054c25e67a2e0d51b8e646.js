        ...
        constructor() {
          ...
          this._applyNow = () => {
            ...
            if (aspect >= this.threshold) {
              // 宽/高 ≥ 1：保持 1:1，不再变化 高
              var side = this.designWidth;
              var policy = ResolutionPolicy.FIXED_HEIGHT;
              var key = policy + ":" + side + "x" + side;
              ...
              // 宽/高 < 1：正常缩放  宽
              var dw = this.designWidth,
                  dh = this.designHeight;
              var _policy = ResolutionPolicy.FIXED_WIDTH; // 随宽

              var _key = _policy + ":" + dw + "x" + dh;

              if (this._lastKey !== _key) {
                this._lastKey = _key;
                view.setDesignResolutionSize(dw, dh, _policy);
                this.scheduleOnce(this._refreshUIOnce, 0);
          ...
          this._refreshUIOnce = () => {
            ...
            for (var w of scene.getComponentsInChildren(Widget)) w.updateAlignment();

            for (var l of scene.getComponentsInChildren(Layout)) l.updateLayout();
          };
        }
