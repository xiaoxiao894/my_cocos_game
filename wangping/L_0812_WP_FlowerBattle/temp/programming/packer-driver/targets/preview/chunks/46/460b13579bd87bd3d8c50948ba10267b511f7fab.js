    ...
    execute: function () {
      ...
      _export("UIJoyStick", UIJoyStick = (_dec = ccclass('UIJoyStick'), _dec2 = property(Sprite), _dec3 = property(Sprite), _dec4 = property(Node), _dec5 = property(CCFloat), _dec(_class = (_class2 = class UIJoyStick extends Component {
        ...
        onTouchStart(event) {
          ...
          var x = event.touch.getUILocationX();
          var y = event.touch.getUILocationY();
        ...
        onTouchMove(event) {
          ...
          var x = event.touch.getUILocationX();
          var y = event.touch.getUILocationY();
          var worldPos = new Vec3(x, y, 0);
          var localPos = v3();
          this.joyStickBg.node.inverseTransformPoint(localPos, worldPos);
          var thumbnailPos = v3();
          var len = localPos.length();
          localPos.normalize();
          Vec3.scaleAndAdd(thumbnailPos, v3(), localPos, math.clamp(len, 0, this.radius));
          this.thumbnail.node.setPosition(thumbnailPos); // 计算目标值

          var targetHorizontal = this.thumbnail.node.position.x / this.radius;
          var targetVertical = this.thumbnail.node.position.y / this.radius; // 使用线性插值进行平滑处理
        ...
        update(dt) {
          ...
          var xRaw = this.amplitudeX * Math.sin(this.t);
          var yRaw = this.amplitudeY * Math.sin(this.t) * Math.cos(this.t);
          var x = yRaw;
          var y = -xRaw;
          var offset = new Vec3(x, y, 0);
          this.thumbnail.node.setPosition(offset);

          if (this.hand) {
            var handPos = this.handStartPos.clone().add(offset);
