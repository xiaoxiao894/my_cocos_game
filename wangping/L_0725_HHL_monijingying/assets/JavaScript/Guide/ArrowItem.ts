...
export class ArrowItem extends Component {
    ...
    update(dt: number) {
        ...
        this._tempPos.set(this._targetVec.x, this._targetVec.y + this._heightOffset + floatY, this._targetVec.z);
        this.arrow.setWorldPosition(this._tempPos);
    ...
    /** 更新目标位置 */
    private updateTargetVec(data:any){
        if(!data){
        ...
        if(data.pos&&this._targetVec&&Vec3.equals(data.pos,this._targetVec)){
            return;
        }

        this._targetVec = data.pos;
