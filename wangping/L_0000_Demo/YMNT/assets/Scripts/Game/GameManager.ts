import { _decorator, Component, geometry, PhysicsSystem, Vec2, Vec3, Node, Prefab, NodePool, instantiate, Collider, ITriggerEvent, game } from 'cc';
...
export class GameManager extends Component {
    ...
    private onUnlockItem(type: BuildingType): void {
        if(type === BuildingType.TransLater){
        ...
        if(type === BuildingType.Salesclerk){
            if(this.unlockItemMap.get(BuildingType.Gun)?.UnlockState === BuildUnlockState.NoActive){
                app.event.emit(CommonEvent.SetUnlockStatue, {type: BuildingType.Gun, state: BuildUnlockState.Active});
            }
        }
    }
