import { _decorator, Component, CCFloat, Vec3, RigidBody, ERigidBodyType, v3, Node, Collider, CCBoolean, Quat } from 'cc';
...
export class MovementComponent extends BaseComponet {
    ...
    needCheckRotation = true;

    /**
     * 更新旋转值，对于需要检测的角色，只保留Y轴的旋转，避免X轴和Z轴由于物理影响导致的模型倾斜
     * @param dt 帧间隔时间
     */
    protected updateRotation(dt: number) {
        if (this.needCheckRotation) {
            // 获取当前节点的四元数旋转
            ...
            if (rotation.x !== 0 || rotation.y !== 0) {
                ...
                // 只保留Y轴的旋转，将X轴和Z轴的旋转置为0
                eulerAngles.x = 0;
                eulerAngles.z = 0;

                // 将更新后的欧拉角转换回四元数
                const newRotation = new Quat();
                Quat.fromEuler(newRotation, eulerAngles.x, eulerAngles.y, eulerAngles.z);
