...
export class ContainerItem {
    ...
    @property({
        displayName: '达到堆叠上限后是否还可拾取'
    })
    public canPickupAfterMax: boolean = true;
}
...
export class PickupComponent extends BaseComponet {
    ...
    // 记录正在飞来但还未实际进入容器的物品数量
    private itemMapInAir: Map<ObjectType, number> = new Map();
    ...
    private checkAndPickupItems() {
        ...
        const pickableItems = nearbyItemsWithDistance.filter(itemData => {
            const check = this.canPickupContainer(itemData.item.objectType);
            // 必须有对应容器且容器允许拾取
            return check;
        });
    ...
    public pickupItem(item: DropItemCom) {
        ...
        if (!this.canPickupContainer(item.objectType)) {
            return;
        }

        // 增加飞行中物品计数
        const currentInAirCount = this.itemMapInAir.get(item.objectType) || 0;
        this.itemMapInAir.set(item.objectType, currentInAirCount + 1);
    ...
    /** 判定指定物品类型的容器是否还可拾取更多物品 */
    private canPickupContainer(itemType: ObjectType): boolean {
        const container = this.findMatchingContainer(itemType);
        if (!container) return false;
        if (container.canPickup) {
            if (!container.canPickupAfterMax) {
                return this.getContainerItemCount(container.itemType) < container.itemMaxCount;
            }
            return true;
        }
        return false;
    ...
    private arrangeContainers() {
        ...
        for (const container of this.itemContainer) {
            ...
            if (itemCount > 0) {
                // 只有数量大于0的容器才参与排列
                const zPosition = 0.75 + (arrangeIndex * 0.9);
    ...
    private handlePickupEffect(item: DropItemCom) {
        // 确认已拾取，减少飞行中物品计数
        const currentInAirCount = this.itemMapInAir.get(item.objectType) || 0;
        if (currentInAirCount <= 0) {
            console.warn("没有飞行中的物品，但拾取成功了一个？GM指令？", item.objectType);
        }
        this.itemMapInAir.set(item.objectType, Math.max(0, currentInAirCount - 1));
    ...
    private addItem(item: DropItemCom, count: number = 1) {
        ...
        if (type === ObjectType.DropItemCoin) {
            app.audio.playEffect('resources/MQBY/收取金币');
        } else {
            app.audio.playEffect('resources/MQBY/收取资源');
        }
    ...
    public consumeItem(type: ObjectType, count: number): Node[] {
        // 获取当前容器内物品数量，不包含飞行中物品
        const inContainerCount = this.itemMap.get(type) || 0;
        
        // 检查是否有足够的物品
        if (inContainerCount < count) {
            // app.log.warn(`物品不足，无法消耗物品类型: ${type}, 数量: ${count}`);
            return [];
        }
        ...
        // 更新容器内物品数量统计
        const newCount = inContainerCount - count;
        this.itemMap.set(type, newCount);

        const inAirCount = this.itemMapInAir.get(type) || 0;
        this.node.emit(ComponentEvent.UPDATE_ITEM_COUNT, type, newCount + inAirCount);
        ...
        if (matchingContainer && matchingContainer.itemNode) {
            ...
            const outsideCount = inContainerCount - containerCount.length;
    ...
    public getItemCount(type: ObjectType): number {
        const inContainerCount = this.itemMap.get(type) || 0;
        const inAirCount = this.itemMapInAir.get(type) || 0;
        return inContainerCount + inAirCount;
    }

    /**
     * 获取指定物品类型已在容器中的数量，排除还未飞进容器的部分
     * @param type 物品类型
     * @returns 指定物品类型在容器中的数量
     */
    public getItemInContainerCount(type: ObjectType): number {
    ...
    public async magnetPickup(magnetRange?: number): Promise<void> {
        ...
        const pickableItems = itemsWithDistance.filter(itemData => {
            // 必须有对应容器且容器允许拾取
            return this.canPickupContainer(itemData.item.objectType);
        });
    ...
    public canPickupItem(itemOrType: DropItemCom | ObjectType): boolean {
        ...
        // 必须有容器且容器允许拾取
        return this.canPickupContainer(itemType);;
    }
