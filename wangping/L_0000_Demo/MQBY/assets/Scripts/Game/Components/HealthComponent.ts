...
import { ColorEffectType, ObjectType } from '../../common/CommonEnum';
...
export class HealthComponent extends BaseComponet {
    ...
    /** 是否显示伤害数字，默认显示 */
    @property({displayName: '是否显示伤害数字'})
    protected showDamageNum: boolean = true;
     ...
     * @returns 是否死亡
     */
    public takeDamage(damageData : DamageData, damageColor?: Color): boolean {
        // 已经死亡或处于免疫状态则不受伤害
        if (this.isDead ) {
            return false;
        }

        if(this.isImmune && !damageData.ignoreImmunity){
            return false;
        }
        ...
        if(this.showDamageNum){
            ...
            const showPos = this.node.getWorldPosition().add(new Vec3(0, 0.5, 0.5));
            damageNum.showDamage(damageData.damage, showPos, damageColor || this.defaultDamageColor);

            // 同时显示掉血特效
            // manager.effect.playEffect(manager.effect.effectType.GreenDamage, showPos);
        }
