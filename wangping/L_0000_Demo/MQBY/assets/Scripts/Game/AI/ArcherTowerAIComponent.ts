...
export enum ArcherTowerAIState {
    Idle = 'idle',
    Attack = 'attack',
    Dead = 'dead',
}

@ccclass('ArcherTowerAIComponent')
export class ArcherTowerAIComponent extends BaseAIComponent {

    private _stateMachine: StateMachine<ArcherTowerAIState> = new StateMachine<ArcherTowerAIState>();
    ...
    private onCharacterDead(): void {
        this._stateMachine.changeState(ArcherTowerAIState.Dead);
    }
    ...
    protected makeDecision(): void {
        ...
        switch (this._stateMachine.currentState) {
            case ArcherTowerAIState.Idle:
                let target = this.character.GetAttackTarget();
                if (target) {
                    this._stateMachine.changeState(ArcherTowerAIState.Attack);
                }
                break;
            case ArcherTowerAIState.Attack:
                break;
            case ArcherTowerAIState.Dead:
                // 死亡状态不需要做任何决策
                break;
        }
    }
