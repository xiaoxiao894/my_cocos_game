...
export class CoinContainer extends Component {
    ...
    @property({ type: Node, displayName: '感应UI节点' })
    public coinSupplyUINode: Node = null!;

    @property({ type: Node, displayName: '引导位置节点' })
    public guidePosNode: Node = null!;
    ...
    private _checkInterval: number = Config.UNLOCK_CHECK_INTERVAL * 0.5;
    ...
    update(dt: number) {
        ...
        while (this.checkTimer >= this._checkInterval) {
            this._checkPickupComponents();
            this.checkTimer -= this._checkInterval; // 减去一个间隔时间而不是重置为0
        }
    }
    ...
    private onTriggerEnter(event: ITriggerEvent) {
        ...
        if (this.pickupComponents.size > 0) {
            this.coinSupplyUINode && !this.coinSupplyUINode.active && (this.coinSupplyUINode.active = true);
        }

        if (node.name == "Hero_Girl") {
            manager.game.hero.InSpArea = true;
        }
    }
    ...
    private onTriggerExit(event: ITriggerEvent) {
        ...
        if (this.pickupComponents.size === 0) {
            this.coinSupplyUINode && this.coinSupplyUINode.active && (this.coinSupplyUINode.active = false);
        }

        if (node.name == "Hero_Girl") {
            manager.game.hero.InSpArea = false;
        }
    }
