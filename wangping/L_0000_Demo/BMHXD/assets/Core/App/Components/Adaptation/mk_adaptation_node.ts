import { Node, _decorator, Canvas, Component, director, Enum, size, Size, Sprite, UITransform, v2, v3, Vec3 } from "cc";
...
@ccclass
@executeInEditMode
export default class mk_adaptation_node extends Component {
	/* --------------- 属性 --------------- */
	/** 编辑器预览 */
	...
	/** 适配模式 */
	@property({ displayName: "适配模式", type: Enum(_mk_adaptation_node.mode) })
	adaptation_mode = _mk_adaptation_node.mode.scale;

	/** 适配来源 */
	@property({ displayName: "适配来源", type: Enum(_mk_adaptation_node.source) })
	...
	custom_adapt_size = size();

	/** 适配类型 */
	@property({ displayName: "适配类型", type: Enum(_mk_adaptation_node.type) })
	...
	/** 最大缩放 */
	@property
	private _max_scale_v3 = v3(1, 1, 1);

	/** 最小缩放 */
	@property
	private _min_scale_v3 = v3(1, 1, 1);
	...
	onEnable(): void {
		...
		if (this.adaptation_source === _mk_adaptation_node.source.canvas) {
			...
			this.node.parent?.on(Node.EventType.SIZE_CHANGED, this._event_node_size_changed, this);
		}
	...
	onDisable(): void {
		if (this.adaptation_source === _mk_adaptation_node.source.canvas) {
			...
			this.node.parent?.off(Node.EventType.SIZE_CHANGED, this._event_node_size_changed, this);
		}
	...
	/** 自适应-展示完 */
	private _adaptive_show_all(design_size_: Size, frame_size_: Size): void {
		// 检查除数是否为零
	...
	/** 自适应-填充满 */
	private _adaptive_fill_up(design_size_: Size, frame_size_: Size): void {
		// 检查除数是否为零
		if (frame_size_.width <= 0 || frame_size_.height <= 0) {
			app.log.warn("mk_adaptation_node: 帧尺寸不能为零");
			return;
	...
	/** 填充宽 */
	private _fill_width(design_size_: Size, frame_size_: Size): void {
		// 检查除数是否为零
	...
	/** 填充高 */
	private _fill_height(design_size_: Size, frame_size_: Size): void {
		// 检查除数是否为零
	...
	/** 默认 */
	private _default(design_size_: Size, frame_size_: Size): void {
		if (this.adaptation_mode === _mk_adaptation_node.mode.scale) {
	...
	/** 自适应 */
	private _auto_adaption(design_size_: Size, frame_size_: Size): void {
		// 检查除数是否为零
		if (frame_size_.width <= 0 || frame_size_.height <= 0) {
			app.log.warn("mk_adaptation_node: 帧尺寸不能为零");
			return;
	...
	update_adaptation(): void {
		...
		try {
			...
			let frame_size: Size;
			/** 容器节点 */
			let layout_node: Node | null = null;
