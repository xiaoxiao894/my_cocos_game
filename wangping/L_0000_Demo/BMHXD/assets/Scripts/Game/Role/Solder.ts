...
export class Solder extends Character {
    ...
    public lookAt(target: Vec3): void {
        ...
        Vec3.subtract(direction, target, this.node.getWorldPosition());
        direction.normalize();
        this.node.emit(ComponentEvent.SET_FACE_DIRECTION_FROM_3D, direction, 10);
    }
    ...
    public GetAttackTarget(): Node | null {
        ...
        // 过滤掉虫卵
        const validEnemies = enemies.filter(enemy => !enemy.node.getComponent(SpiderEgg));
        
        if(validEnemies.length > 0){
            return validEnemies[0].node;            
        }
    ...
    public searchForAttackTarget(searchRadius: number): { node: Node; squaredDistance: number }[] {
        ...
        // 过滤掉虫卵
        const validEnemies = enemies.filter(enemy => !enemy.node.getComponent(SpiderEgg));
        // console.log('enemies', validEnemies.length);
        return validEnemies;
    }
