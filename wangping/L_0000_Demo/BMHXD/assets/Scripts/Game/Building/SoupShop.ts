...
export class SoupShop extends Component {
    ...
    @property({type: Node, displayName: '售货员站立位置点'})
    public salesclerkTarget: Node = null!;
    ...
    private flyingSoupCount: number = 0; // 跟踪飞行中的汤数量
    ...
    private onUse(pickupComponent: PickupComponent) {
        if(pickupComponent.getItemCount(ObjectType.DropItemCornSoup ) > 0){
            ...
            if(consumeList.length > 0){
                ...
                // 标记汤开始飞行
                this.flyingSoupCount++;
                this.isSoupFlying = this.flyingSoupCount > 0;
    ...
    GetCurrWaitPosition(index?: number): Vec3 {
        ...
        return this.waitPos.getWorldPosition().add(v3( 3.5 * index, 0, -3.5 * index));
    }
     ...
     * 检查是否有汤正在飞行中
     */
    public get IsAnyFlyingSoup(): boolean {
        return this.isSoupFlying;
    }

    /**
     * 获取飞行中的汤数量
     */
    public get FlyingSoupCount(): number {
        return this.flyingSoupCount;
    }

    /**
     * 获取汤店的总汤数（包括布局中的和飞行中的）
     */
    public getTotalSoupCount(): number {
        const layoutCount = this.soupLayout.getItemCount();
        return layoutCount + this.flyingSoupCount;
    }
