...
export class HeroAIComponent extends BaseAIComponent {
    public get character(): Hero {
        return this._character as Hero;
    }
    public set character(value: Hero) {
    ...
    protected makeDecision(): void {
        ...
        switch (this._stateMachine.currentState) {
            case HeroAIState.Idle:
                if(this.character.weaponType === WeaponType.FireGun){
                    const isAtHome = this.character.isAtHome();
                    if(!isAtHome || this.character.isFacingCorn()){
