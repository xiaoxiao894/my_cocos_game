...
export class UIJoyStick extends Component {
    ...
    private initStickBgPos: Vec3 = new Vec3();
    ...
    start() {
        ...
        this.initStickBgPos = this.joyStickBg.node.getPosition().clone();
    ...
    onTouchEnd(event: EventTouch) {
        ...
        // this.node.worldPosition = this.initCurNodePos;
        this.joyStickBg.node.setPosition(this.initStickBgPos);
        ...
        // 如果之前有未执行的定时器，先取消
        if (this._restoreUITimer !== null) {
            this.unschedule(this._restoreUITimer);
            this._restoreUITimer = null;
        }

        // 重新安排恢复任务
        this._restoreUITimer = this.scheduleOnce(() => {
            if (this.uiOpacity) this.uiOpacity.opacity = 255;
            if (this.hand) this.hand.active = true;
            DataManager.Instance.isTouching = false;
            this._restoreUITimer = null; // 执行完清理
        }, 4);
    }
