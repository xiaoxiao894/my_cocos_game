...
export class MinionConManager extends Component {
    ...
    // 起点
    birthPoint = new Vec3(-0.657, 0, 12);
    ...
    // 出门路径
    exitRoute = [new Vec3(-0.657, 0, 20), new Vec3(11.11, 0, 20)];
    ...
    goOutMinion() {
        ...
        const weaponTarget = minionWorld.clone().add(new Vec3(0, 2, 1.2));
                ...
                for (let i = 0; i < route.length; i++) {
                    ...
                    moveChain = moveChain
                        .call(() => {
                            ...
                            tween(minion.children[0])
                                .to(0.3, { eulerAngles: new Vec3(0, targetAngle, 0) }, { easing: 'sineInOut' })
                                .start();

                            if (i === 1) {
                                const LSide = find("ThreeDNode/Map/Fences/Scene1/LSide");
                                const lDoor = LSide?.getChildByName("L_Door");
                                if (lDoor) {
                                    const doorL = lDoor.getChildByName("Door_Left");
                                    const doorR = lDoor.getChildByName("Door_Right");
                                    DataManager.Instance.player.getComponent(Actor).openDoorBySide(doorL, doorR, "L");
                                }
                            }
                        })
                    ...
                    .call(() => {
                        ...
                        if (this._activeMovingMinions.size === 0) {
                            ...
                            if (lDoor) {
                                const doorL = lDoor.getChildByName("Door_Left");
                                const doorR = lDoor.getChildByName("Door_Right");
                                DataManager.Instance.player.getComponent(Actor).closeDoorBySide(doorL, doorR, "L", true);
                            }
                        }
    ...
    public hasTarget(node: Node) {
        ...
        // return this._targetMonsters.includes(node);
    }
}